...
def create_assignment_event_on_change(sender, instance, created, **kwargs):
    ...
    try:
        ...
        event = Event.objects.create(
            ...
            # Assigned to
            assigned_to_type=map_to_actor_type(current_type) if current_type != AssigneeTypeChoices.UNASSIGNED else None,
...
def _trigger_chatflow_start_session(contact, chatflow_id: int):
    ...
    try:
        ...
        if not ChatFlowModel.objects.filter(id=chatflow_id, is_active=True).exists():
            logger.warning(
                f"ChatFlow {chatflow_id} is inactive, skipping session start for contact {contact.id}"
            )
            return
        ...
        logger.info(
            f"Triggered ChatFlow {chatflow_id} start_session for contact {contact.id} "
            f"(queued as Celery task)"
        )
...
def create_status_change_event(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.info(
            f"Created status change event {event.pk} for contact {instance.pk}: "
