...
class ContactCSVUploadSerializer(serializers.Serializer):
    ...
    file = serializers.FileField(
        required=True,
        help_text="CSV file with contacts. Required column: phone. Optional: first_name, last_name, tag"
    )
    skip_duplicates = serializers.BooleanField(
        required=False,
        default=True,
        help_text="Skip contacts with phone numbers that already exist (default: true)"
    )
    ...
    def validate_file(self, value):
        ...
        if not filename.endswith('.csv'):
            raise serializers.ValidationError(
                "Invalid file format. Please upload a CSV file (.csv extension)."
            )
...
class ContactBulkCreateSerializer(serializers.Serializer):
    ...
    skip_duplicates = serializers.BooleanField(
        required=False,
        default=True,
        help_text="Skip contacts with phone numbers that already exist"
    )
    default_tag = serializers.CharField(
        required=False,
        default='',
