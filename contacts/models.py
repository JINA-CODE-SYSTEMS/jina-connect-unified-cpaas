...
class TenantContact(BaseTenantModelForFilterUser):
    ...
    last_called_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When this contact was last called'
    )
    total_calls = models.PositiveIntegerField(
        default=0,
        help_text='Total number of calls with this contact'
    )
    dnc = models.BooleanField(
        default=False,
        help_text='Do Not Call flag'
    )
    ...
    assigned_to_type = models.CharField(
        ...
        default=AssigneeTypeChoices.UNASSIGNED,
        db_index=True
    )
    assigned_to_id = models.PositiveIntegerField(
        null=True, 
        blank=True,
        help_text="User ID or Bot ID depending on assigned_to_type"
    )
    ...
    assigned_by_id = models.PositiveIntegerField(
        null=True,
        blank=True,
        help_text="User ID, Bot ID, or ChatFlow ID depending on assigned_by_type"
    )
    ...
    assignment_note = models.TextField(
        blank=True,
        null=True,
        help_text="Note or instructions from assigner to assignee"
    )
