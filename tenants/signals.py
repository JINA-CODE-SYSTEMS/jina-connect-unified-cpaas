    ...
    if created:
        ...
        if instance.bsp == BSPChoices.GUPSHUP:
            if settings.CELERY_BROKER_URL:
                ...
                logger.info(
                    f"Skipping auto webhook registration for app {instance.pk} "
                    f"(no Celery broker configured)"
                )
...
def upload_media_to_whatsapp(tenant_media_instance, method='upload_media'):
    ...
    if not wa_app:
        raise ValueError(
            f"No WA app found for tenant {tenant.id}. "
            "Cannot upload media to WhatsApp."
        )
    ...
    if not media_file:
        raise ValueError(
            f"No media file on TenantMedia {tenant_media_instance.id}."
        )

    filename = os.path.basename(media_file.name) if media_file.name else "upload"

    logger.info(
        f"Uploading TenantMedia {tenant_media_instance.id} via "
        f"{adapter.__class__.__name__} (BSP={wa_app.bsp})"
    )
    ...
    logger.info(
        f"Read {len(file_bytes)} bytes from storage for "
        f"TenantMedia {tenant_media_instance.id}"
    )
    ...
    if result.success:
        ...
        logger.info(
            f"Upload succeeded for TenantMedia {tenant_media_instance.id}: "
            f"handle_id={handle_id}"
        )
    else:
        logger.warning(
            f"Upload failed for TenantMedia {tenant_media_instance.id}: "
...
def update_tenant_balance(sender, instance, created, **kwargs):
    if created:
        ...
        if instance.transaction_type == TransactionTypeChoices.SUCCESS_RECHARGE:
            ...
            instance.tenant.balance  = money_currency_converter_and_adder(instance.tenant.balance, instance.amount, add=False)
