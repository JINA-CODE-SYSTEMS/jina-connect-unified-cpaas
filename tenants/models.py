from abstract.models import (BaseEntity, BaseModelWithOwner,
                             BaseTenantModelForFilterUser, BaseWallet)
...
class Tenant(BaseEntity, BaseWallet, BaseTenantModelForFilterUser):
    ...
    country = models.CharField(max_length=2, blank=True, null=True, help_text="ISO 3166-1 alpha-2 country code (e.g., IN, US)")
    state = models.CharField(max_length=10, blank=True, null=True, help_text="ISO 3166-2 subdivision code (e.g., MH, CA)")
...
class TenantWAApp(BaseTenantModelForFilterUser):
    ...
    wa_number = models.CharField(max_length=15, validators=[validate_phone_with_series], help_text="Enter in international format, e.g., +14155552671. Do not include spaces or dashes.")
    ...
    esf_url = models.URLField(max_length=2048, null=True, blank=True, help_text="Embedded Signup Flow URL for WhatsApp onboarding")
    esf_url_expires_at = models.DateTimeField(null=True, blank=True, help_text="ESF URL expiration time (valid for 4 days)")
...
class WABAInfo(BaseTenantModelForFilterUser):
    ...
    messaging_limit = models.CharField(
        max_length=20, 
        choices=MessagingLimit.choices,
        null=True,
        blank=True
    )
    mm_lite_status = models.CharField(
        max_length=20, 
        choices=MMLiteStatus.choices,
        null=True,
        blank=True
    )
    ...
    phone_quality = models.CharField(
        max_length=20, 
        choices=PhoneQuality.choices,
        null=True,
        blank=True
    )
    throughput = models.CharField(
        max_length=20, 
        choices=Throughput.choices,
        null=True,
        blank=True
    )
    ...
    can_send_message = models.CharField(
        max_length=20, 
        choices=CanSendMessage.choices,
        null=True,
        blank=True
    )
    ...
    additional_info = models.JSONField(
        default=list,
        blank=True,
        help_text="List of additional information messages"
    )
    ...
    def update_from_api_response(cls, wa_app, api_response):
        ...
        waba_info, created = cls.objects.get_or_create(
            wa_app=wa_app
        )
...
class RolePermission(models.Model):
    ...
    role = models.ForeignKey(
        TenantRole, on_delete=models.CASCADE, related_name="permissions"
    )
...
class TenantMedia(BaseTenantModelForFilterUser):
    ...
    wa_handle_id = models.JSONField(null=True, blank=True, help_text="Handle ID returned by WhatsApp (Gupshup) after media upload.")
    media_id = models.TextField(null=True, blank=True, help_text="Media ID returned by WhatsApp (Gupshup) after media upload.")    
    auto_convert = models.BooleanField(default=False, help_text="If enabled, media will be auto-converted to WhatsApp compatible format upon upload.")
...
class BrandingSettings(models.Model):
    ...
    favicon = models.ImageField(
        ...
        null=True,
        blank=True,
        help_text="Favicon PNG image (583x583 px)"
    )
    ...
    primary_logo_url = models.URLField(
        ...
        null=True,
        blank=True,
        help_text="External URL for primary logo"
    )
    ...
    secondary_logo_url = models.URLField(
        ...
        null=True,
        blank=True,
        help_text="External URL for secondary logo"
    )
