...
from tenants.validators import MediaValidationError, WhatsAppMediaValidator
...
class TenantGupshupAppNestedSerializer(serializers.ModelSerializer):
    ...
    wa_authentication_message_price = MoneyField(
        decimal_places=2, max_digits=15, source="authentication_message_price", read_only=True
    )
    wa_authentication_message_price_currency = serializers.CharField(
        source="authentication_message_price_currency", read_only=True
    )
    wa_marketing_message_price = MoneyField(
        decimal_places=2, max_digits=15, source="marketing_message_price", read_only=True
    )
    wa_marketing_message_price_currency = serializers.CharField(
        source="marketing_message_price_currency", read_only=True
    )
    wa_utility_message_price = MoneyField(
        decimal_places=2, max_digits=15, source="utility_message_price", read_only=True
    )
...
class TenantMediaSerializer(BaseSerializer):
    ...
    media = serializers.FileField(
        required=True, help_text="Upload media file. Auto-converts: HEIC, WebP, MOV, AVI, WAV to WhatsApp formats"
    )
    ...
    class Meta(BaseSerializer.Meta):
        ...
        fields = "__all__"
        extra_kwargs = {
            "media": {"help_text": "Upload a media file", "style": {"base_template": "file.html"}},
...
class TenantGupshupAppsSerializer(BaseSerializer):
    ...
    wa_authentication_message_price = MoneyField(
        decimal_places=2, max_digits=15, write_only=True, source="authentication_message_price"
    )
    wa_marketing_message_price = MoneyField(
        decimal_places=2, max_digits=15, write_only=True, source="marketing_message_price"
    )
    wa_utility_message_price = MoneyField(
        decimal_places=2, max_digits=15, write_only=True, source="utility_message_price"
    )
...
class TenantRegistrationSerializer(serializers.Serializer):
    ...
    tenant_country = serializers.CharField(
        max_length=2, required=False, allow_blank=True, help_text="ISO 3166-1 alpha-2 country code (e.g., IN, US)"
    )
    tenant_state = serializers.CharField(
        max_length=10, required=False, allow_blank=True, help_text="ISO 3166-2 subdivision code (e.g., MH, CA)"
    )
    tenant_industry = serializers.ChoiceField(
        choices=IndustryChoices.choices, required=False, allow_blank=True, help_text="Industry/sector of the tenant"
    )
    ...
    phone = serializers.CharField(
        max_length=20, help_text="User's phone number with country code (e.g., +919876543210)"
    )
...
class TransferOwnershipSerializer(serializers.Serializer):
    ...
    def validate_target_user_id(self, value):
        ...
        target_tu = (
            TenantUser.objects.select_related("role").filter(tenant=tenant, user_id=value, is_active=True).first()
        )
        if not target_tu:
            raise serializers.ValidationError("Target user is not an active member of this tenant.")
        if not target_tu.role or target_tu.role.slug != "admin":
            raise serializers.ValidationError("Ownership can only be transferred to a user with the ADMIN role.")

        # Requester must be OWNER
        requester_tu = (
            TenantUser.objects.select_related("role").filter(tenant=tenant, user=request.user, is_active=True).first()
        )
        if not requester_tu or not requester_tu.role or requester_tu.role.slug != "owner":
            raise serializers.ValidationError("Only the current OWNER can transfer ownership.")
...
class CreateRoleSerializer(serializers.Serializer):
    ...
    def validate_slug(self, value):
        ...
        if TenantRole.objects.filter(tenant=tenant, slug=value).exists():
            raise serializers.ValidationError("A role with this slug already exists in this tenant.")
        return value
    ...
    def validate(self, data):
        ...
        if custom_count >= 20:
            raise serializers.ValidationError("Maximum of 20 custom roles per tenant reached.")
        return data
