...
class HostWalletViewSet(viewsets.ViewSet):
    ...
    def dashboard(self, request):
        ...
        tenants_last_month = Tenant.objects.filter(is_active=True, created_at__lt=current_month_start).count()

        # Tenants that existed at end of previous month
        tenants_previous_month = Tenant.objects.filter(is_active=True, created_at__lt=previous_month_start).count()
        ...
        current_month_messages = Messages.objects.filter(timestamp__gte=current_month_start).count()

        # Previous month messages
        previous_month_messages = Messages.objects.filter(
            timestamp__gte=previous_month_start, timestamp__lt=current_month_start
        ).count()
        ...
        platform_stats = Messages.objects.values("platform").annotate(count=Count("id"))
    ...
    def message_stats(self, request):
        ...
        return Response(
            {"whatsapp_sent": whatsapp_sent, "sms_sent": sms_sent, "total_sent": total_sent, "date_range": date_range}
        )
    ...
    def latest_recharges(self, request):
        ...
        transactions = (
            TenantTransaction.objects.filter(transaction_type__in=recharge_types)
        ...
        return Response({"recharges": recharges, "total_count": len(recharges)})

    @action(
        detail=False, methods=["get"], url_path="alerts", permission_classes=[IsAuthenticated, TenantRolePermission]
    )
    def alerts(self, request):
            ...
            alerts.append(
                {
                    ...
                    "message": (
                        f"Your balance is {tenant.total_balance}. Recharge immediately to continue sending messages."
                    ),
            ...
            alerts.append(
                {
                    ...
                    "message": (f"Template '{tpl_name}' was rejected by Meta. Review and resubmit."),
        ...
        broadcast_stats = (
            BroadcastMessage.objects.filter(broadcast__tenant=tenant, broadcast__created_at__gte=seven_days_ago)
            .exclude(
                status__in=[
