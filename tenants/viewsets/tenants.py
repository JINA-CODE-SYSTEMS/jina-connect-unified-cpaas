...
from tenants.serializers import (MyPermissionsSerializer,
                                 TenantLimitedSerializer,
                                 TenantRegistrationSerializer,
                                 TenantSerializer, TransferOwnershipSerializer)
...
class TenantViewSet(BaseTenantModelViewSet):
    ...
    def get_queryset(self):
        ...
        """
        return Tenant.objects.prefetch_related('wa_apps').annotate(
    ...
    def my_permissions(self, request):
        ...
        granted = set(
            RolePermission.objects.filter(role=role, allowed=True)
    ...
    def transfer_ownership(self, request):
        ...
        requester_tu = (
            TenantUser.objects.select_related("tenant", "role")
    ...
    @action(detail=False, methods=['post'], url_path='register', url_name='register', serializer_class=TenantRegistrationSerializer, permission_classes=[AllowAny])
    ...
    def resend_verification(self, request):
        ...
        return Response({
            'message': message if success else 'If an account exists with this email, a verification link has been sent.'
        }, status=status.HTTP_200_OK)
    ...
    @action(detail=False, methods=['post'], url_path='validate-verification-token', url_name='validate-verification-token')
