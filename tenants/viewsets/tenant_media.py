...
from tenants.validators import get_audio_rules, get_document_rules, get_image_rules, get_video_rules
...
class TenantMediaViewSet(BaseTenantModelViewSet):
        ...
        return Response(
            {
                ...
                "message": "All conversion tools are available!"
                if all_available
                else "Some conversion tools are missing. See missing_tools for details.",
            }
        )
    ...
    def upload_to_wa(self, request, pk=None):
        ...
        try:
            ...
            logger.exception(f"upload_to_wa unexpected error for TenantMedia {instance.id}: {exc}")
            return Response(
