...
class WhatsAppMediaValidator:
    ...
    def validate_extension(cls, file):
        ...
        if ext not in cls.ALL_SUPPORTED_EXTENSIONS:
            raise MediaValidationError(
                f"Unsupported file extension '{ext}'. Allowed extensions: {', '.join(cls.ALL_SUPPORTED_EXTENSIONS)}"
            )
    ...
    def validate_mime_type(cls, file, expected_category=None):
        ...
        if not detected_mime:
            ...
            if not MAGIC_AVAILABLE:
                ...
                raise MediaValidationError("Could not detect file MIME type. The file may be corrupted or invalid.")
    ...
    def validate_file_size(cls, file, is_template=True):
        ...
        if media_type == "document":
            ...
            max_size = (
                MediaTypeConfig.IMAGE["max_size_bytes_template"]
                if is_template
                else MediaTypeConfig.IMAGE["max_size_bytes_message"]
            )
            max_size_mb = (
                MediaTypeConfig.IMAGE["max_size_mb_template"]
                if is_template
                else MediaTypeConfig.IMAGE["max_size_mb_message"]
            )
        elif media_type == "video":
            max_size = (
                MediaTypeConfig.VIDEO["max_size_bytes_template"]
                if is_template
                else MediaTypeConfig.VIDEO["max_size_bytes_message"]
            )
            max_size_mb = (
                MediaTypeConfig.VIDEO["max_size_mb_template"]
                if is_template
                else MediaTypeConfig.VIDEO["max_size_mb_message"]
            )
        elif media_type == "audio":
            ...
            try:
                ...
                with zipfile.ZipFile(zip_buffer, "r") as zf:
                    ...
                    if "[Content_Types].xml" not in namelist:
                        raise MediaValidationError(
                            "Invalid DOCX file structure. The file may be corrupted or is not a valid Word document."
                        )

                    # Check for DOCX-specific directory
                    has_word_dir = any(name.startswith("word/") for name in namelist)
                    if not has_word_dir:
                        raise MediaValidationError("Invalid DOCX file. Missing required Word document structure.")
                ...
                raise MediaValidationError("Invalid or corrupted DOCX file. Please re-export the document.")
            except ImportError:
        ...
        try:
            ...
            try:
                ...
                with zipfile.ZipFile(zip_buffer, "r") as zf:
                    ...
                    if "[Content_Types].xml" not in namelist:
                        raise MediaValidationError(
                            "Invalid XLSX file structure. The file may be corrupted or is not a valid Excel document."
                        )

                    # Check for XLSX-specific directory
                    has_xl_dir = any(name.startswith("xl/") for name in namelist)
                    if not has_xl_dir:
                        raise MediaValidationError("Invalid XLSX file. Missing required Excel document structure.")
                ...
                raise MediaValidationError("Invalid or corrupted XLSX file. Please re-export the document.")
            except ImportError:
                pass
        ...
        try:
            ...
            try:
                ...
                with zipfile.ZipFile(zip_buffer, "r") as zf:
                    ...
                    if not has_ppt_dir:
                        raise MediaValidationError("Invalid PPTX file. Missing required PowerPoint document structure.")
                    ...
                    has_embedded_media = any(
                        any(n.lower().endswith(ext) for ext in video_extensions) for n in media_files
                    )
                ...
                raise MediaValidationError("Invalid or corrupted PPTX file. Please re-export the document.")
            except ImportError:
                pass
    ...
    def validate_image(cls, file):
        ...
        try:
            ...
            if img.mode == "CMYK":
                raise MediaValidationError(
                    "CMYK color format is not supported. Please convert the image to RGB color format."
                )
            ...
            raise MediaValidationError(f"Invalid or corrupted image file: {str(e)}")
    ...
    def validate_video(cls, file):
        ...
        try:
            ...
            if b"ftyp" not in header:
                raise MediaValidationError("Invalid MP4 file. The file does not have a valid MP4 header.")
...
class WhatsAppMediaSerializerValidator:
    ...
    def __call__(self, value):
        try:
            WhatsAppMediaValidator.validate(value, is_template=self.is_template)
            return value
        except MediaValidationError as e:
