...
class TelegramContactsViewSet(ContactsViewSet):
    """Telegram-scoped contacts API."""

    queryset = TenantContact.objects.all()
    serializer_class = TenantContactSerializer

    def get_queryset(self):
        return super().get_queryset().filter(source__in=[ContactSource.TELEGRAM, ContactSource.MANUAL])
