...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--exclude", nargs="*", default=[], help="List of apps to exclude from migration removal")
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        if excluded_apps:
            self.stdout.write(self.style.WARNING(f"📋 Excluding apps: {', '.join(excluded_apps)}"))

        self.stdout.write(self.style.SUCCESS(f"🎯 Django project apps: {', '.join(project_apps)}"))
        self.stdout.write(self.style.SUCCESS(f"📋 Target apps (after exclusions): {', '.join(target_apps)}"))
            ...
            try:
                ...
                if os.path.exists(migrations_dir):
                    ...
                    if migration_files:
                        ...
                        self.stdout.write(f"📁 {app_name}: {len(migration_files)} migration files found")
                ...
                self.stdout.write(self.style.ERROR(f"❌ Error processing app {app_name}: {e}"))
        ...
        if not force:
            ...
            self.stdout.write(self.style.ERROR("Make sure you have backups of your data."))
