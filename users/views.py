...
def android_asset_links(request):
    ...
    android_sha256_fingerprints = getattr(
        settings, 
...
def verify_email_deep_link(request, token):
    ...
    ios_app_store_url = getattr(
        settings, 
        'IOS_APP_STORE_URL', 
        'https://apps.apple.com/app/jina-connect/id123456789'
    )
    android_play_store_url = getattr(
        settings, 
        'ANDROID_PLAY_STORE_URL', 
        'https://play.google.com/store/apps/details?id=com.jinacode.jinaconnect'
    )
    
    # Validate token exists (optional - for better UX)
    token_valid = EmailVerificationToken.objects.filter(
        token=token, 
        is_used=False
    ).exists()
...
def reset_password_deep_link(request, token):
    ...
    # Validate token exists (optional - for better UX)
    token_valid = PasswordResetToken.objects.filter(
        token=token, 
        is_used=False
    ).exists()
