...
class NodeButton(BaseModel):
    ...
    title: Optional[str] = Field(
        None, min_length=1, max_length=20, description="Button display text (alias for text, used by message nodes)"
    )
...
class NodeData(BaseModel):
    ...
    template_id: Optional[Union[int, str]] = Field(
        None, description="Associated WhatsApp template ID (integer or UUID string)"
    )
...
class ReactFlowData(BaseModel):
    ...
    def validate_flow_structure(self):
        ...
        for edge in edges:
            ...
            has_quick_reply = any(btn.type == "QUICK_REPLY" for btn in node_buttons)
            ...
            has_button_ref = button_id or button_text or (source_handle and source_handle.startswith("button-"))
