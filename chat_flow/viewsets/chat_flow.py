...
from contacts.models import TenantContact, AssigneeTypeChoices
from tenants.permission_classes import TenantRolePermission

from ..models import ChatFlow, ChatFlowEdge
from ..serializers import (ApprovedTemplateSerializer,
                           ChatFlowCreateUpdateSerializer, ChatFlowSerializer)
...
class ChatFlowViewSet(BaseTenantModelViewSet):
    ...
    def _check_active_sessions_for_flow_data(self, request):
        ...
        if 'flow_data' in request.data:
            ...
            active_count = UserChatFlowSession.objects.filter(
                flow=instance, is_active=True
            ).count()
            if active_count > 0:
                return Response(
                    {
    ...
    def deactivate(self, request, pk=None):
            ...
            return Response(
                {
                    ...
                    'flow_id': instance.id,
                    'message': 'Flow is already inactive.'
                },
        ...
        active_sessions = UserChatFlowSession.objects.filter(
            flow=instance, is_active=True
        )
        ...
        active_sessions.update(
            is_active=False,
            ended_at=now,
            cancellation_reason=cancellation_reason or 'Flow deactivated'
        )
        ...
        TenantContact.objects.filter(
            assigned_to_type=AssigneeTypeChoices.CHATFLOW,
            assigned_to_id=instance.id
        ).update(
        ...
        logger.info(
            ...
            instance.id, sessions_ended, cancellation_reason or 'N/A'
        )
    ...
    def approved_templates(self, request):
        ...
        """
        templates = WATemplate.objects.filter(
    ...
    def validate_flow(self, request):
        ...
        if not flow_data or not isinstance(flow_data, dict):
            raise drf_serializers.ValidationError(
                {'flow_data': 'A valid flow_data JSON object is required.'}
            )
        ...
        for key in required_keys:
            if key not in flow_data:
                raise drf_serializers.ValidationError(
                    {'flow_data': f"flow_data must contain '{key}' field."}
                )
    ...
    def validate_node(self, request):
        ...
        if not node or not isinstance(node, dict):
            raise drf_serializers.ValidationError(
                {'node': 'A valid node JSON object is required.'}
            )

        if 'id' not in node:
            raise drf_serializers.ValidationError(
                {'node': "Node must have an 'id' field."}
            )
