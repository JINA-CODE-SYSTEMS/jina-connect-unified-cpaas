...
class ChatFlowSerializer(BaseSerializer):
    ...
    start_template_name = serializers.SlugRelatedField(
        slug_field="element_name", source="start_template", read_only=True
    )
    ...
    def get_active_session_count(self, obj):
        ...
        return UserChatFlowSession.objects.filter(flow=obj, is_active=True).count()
...
class ChatFlowNodeSerializer(BaseSerializer):
    ...
    template_name = serializers.SlugRelatedField(
        queryset=WATemplate.objects.filter(status=S.APPROVED), slug_field="element_name", source="template"
    )
...
class ChatFlowEdgeSerializer(BaseSerializer):
    ...
    source_template_name = serializers.SlugRelatedField(
        slug_field="element_name", source="source_node.template", read_only=True
    )
    target_template_name = serializers.SlugRelatedField(
        slug_field="element_name", source="target_node.template", read_only=True
    )
...
class ChatFlowCreateUpdateSerializer(BaseSerializer):
    ...
    def validate(self, data):
        ...
        if not isinstance(flow_data, dict):
            raise serializers.ValidationError({"flow_data": "Must be a valid JSON object."})
    ...
    def update(self, instance, validated_data):
        ...
        try:
            updated = ChatFlowProcessor.update_flow_from_reactflow(flow=instance, flow_data=flow_data, tenant=tenant)
            try:
...
class ButtonClickSerializer(serializers.Serializer):
    ...
    user_phone = serializers.CharField(
        max_length=20, help_text="User's WhatsApp phone number in international format (e.g., +1234567890)"
    )
    button_id = serializers.CharField(max_length=255, help_text="Unique identifier for the button that was clicked")
    button_text = serializers.CharField(
        max_length=255, required=False, allow_blank=True, help_text="Optional text of the button for analytics purposes"
    )
    ...
    def validate_user_phone(self, value):
        ...
        if len(phone_digits) < 10 or len(phone_digits) > 15:
            raise serializers.ValidationError("Phone number must be between 10 and 15 digits (excluding country code)")
