...
class RuleRegistry:
    ...
    def register(cls, rule_class: Type[FlowRule]) -> Type[FlowRule]:
        ...
        if rule_class.rule_id in cls._rules:
            raise ValueError(
                f"Rule ID '{rule_class.rule_id}' is already registered by {cls._rules[rule_class.rule_id].__name__}"
            )
    ...
    def get_rules_by_category(cls, category: RuleCategory, enabled_only: bool = True) -> List[FlowRule]:
        ...
        """
        rules = [rule_cls() for rule_cls in cls._rules.values() if rule_cls.category == category]
            ...
            docs.append(
                {
                    ...
                    "category": rule_cls.category.value
                    if hasattr(rule_cls.category, "value")
                    else str(rule_cls.category),
                    "severity": rule_cls.severity.value
                    if hasattr(rule_cls.severity, "value")
                    else str(rule_cls.severity),
