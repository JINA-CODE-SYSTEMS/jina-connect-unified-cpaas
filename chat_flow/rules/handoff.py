            ...
            if not isinstance(priority, str) or priority.lower() not in self.VALID_PRIORITIES:
                violations.append(
                    RuleViolation(
                        ...
                        severity=self.severity,
                        node_id=node.get("id"),
                        details={"field": "priority", "value": priority, "valid_values": self.VALID_PRIORITIES},
                    )
                )
            ...
            if target_type != "end":
                violations.append(
                    RuleViolation(
                        ...
                        node_id=node_id,
                        edge_id=edge.get("id"),
                        details={"target_node_id": target_id, "target_node_type": target_type, "expected_type": "end"},
                    )
                )
