...
def start_chatflow_session_task(self, chatflow_id: int, contact_id: int, context: dict = None):
    ...
    logger.info(f"[Task] Starting ChatFlow session: flow={chatflow_id}, contact={contact_id}")
    ...
    try:
        ...
        result = executor.start_session(contact_id=contact_id, context=context or {})
        ...
        logger.exception(f"[Task] Failed to start ChatFlow {chatflow_id} for contact {contact_id}: {e}")
        try:
...
def process_chatflow_input_task(
    self, chatflow_id: int, contact_id: int, user_input: str, additional_context: dict = None
):
    """
    ...
    logger.info(f"[Task] Processing ChatFlow input: flow={chatflow_id}, contact={contact_id}, input='{user_input}'")
    ...
    try:
        ...
        result = executor.process_input(
            contact_id=contact_id, user_input=user_input, additional_context=additional_context
        )
        ...
        logger.exception(f"[Task] Failed to process input for ChatFlow {chatflow_id}, contact {contact_id}: {e}")
        raise
...
def reset_chatflow_session_task(chatflow_id: int, contact_id: int):
    ...
    logger.info(f"[Task] Resetting ChatFlow session: flow={chatflow_id}, contact={contact_id}")
    ...
    try:
        ...
        updated = UserChatFlowSession.objects.filter(flow_id=chatflow_id, contact_id=contact_id, is_active=True).update(
            is_active=False
        )
        ...
        logger.exception(f"[Task] Failed to reset ChatFlow {chatflow_id} session for contact {contact_id}: {e}")
        return {
            "success": False,
            "error": str(e),
        }
...
def continue_flow_after_delay(self, flow_id: int, contact_id: int, next_node_id: str, context: dict = None):
    ...
    logger.info(
        f"[Task] Continuing ChatFlow after delay: flow={flow_id}, contact={contact_id}, next_node={next_node_id}"
    )
    ...
    try:
        ...
        session = UserChatFlowSession.objects.filter(flow_id=flow_id, contact_id=contact_id, is_active=True).first()
        ...
        logger.exception(f"[Task] Failed to continue ChatFlow {flow_id} after delay for contact {contact_id}: {e}")
