...
def send_template_message(
    ...
    try:
        ...
        template = WATemplate.objects.select_related(
            'wa_app', 'wa_app__tenant'
        ).get(id=template_id)
        ...
        if not template.placeholder_mapping:
            ...
            if extracted:
                ...
                logger.info(
                    f"send_template_message: Auto-populated placeholder_mapping "
                    f"for template {template_id}: {extracted}"
                )
        ...
        if template.template_type == TemplateType.CAROUSEL and template.cards:
            ...
            if missing_cards:
                ...
                logger.error(
                    f"send_template_message: {result['error']} "
        ...
        template_number, _ = TemplateNumber.objects.get_or_create(
            gupshup_template=template,
            defaults={'name': template.element_name or template.name or ''}
        )
        ...
        broadcast_message, _ = BroadcastMessage.objects.get_or_create(
            broadcast=broadcast,
            contact=contact,
            defaults={'status': MessageStatusChoices.PENDING}
        )
...
def save_session_to_db(state: FlowState) -> UserChatFlowSession:
    ...
    if created:
        logger.info(
            f"Created new UserChatFlowSession {session.pk} for contact {contact_id} "
            f"at node '{current_node_id}'"
        )
    else:
        logger.info(
            f"Updated UserChatFlowSession {session.pk} for contact {contact_id} "
            f"to node '{current_node_id}'"
        )
...
def create_start_node_handler(node: ChatFlowNode, edges: List[ChatFlowEdge]):
    ...
    def handler(state: FlowState) -> FlowState:
        ...
        logger.info(
            f"Start node '{node_id}': Passthrough mode -> routing to next node: {next_node_id}"
        )
...
def create_template_node_handler(node: ChatFlowNode, edges: List[ChatFlowEdge]):
    ...
    has_quick_reply = any(
        btn.get('type') == 'QUICK_REPLY' for btn in template_buttons
    )
    ...
    if not has_quick_reply:
        for edge in edges:
            if edge.source_node.id == node.id:
                ...
                if edge.button_text in ('__PASSTHROUGH__', None, '') or \
                   not edge.button_text:
    ...
    def handler(state: FlowState) -> FlowState:
        ...
        if resume_target and resume_target == node_id:
            ...
            if pending:
                ...
                logger.info(
                    f"Template node '{node_id}': Restored pending user input '{pending}'"
                )
        ...
        is_resuming = has_quick_reply and user_input and user_input != "__PASSTHROUGH__" and (
            current_node == node_id or awaiting
        )
        ...
        logger.info(
            f"Template node '{node_id}': Sending template {template_id} "
            f"(interactive={has_quick_reply})"
        )
        ...
        if template_id:
            ...
            is_placeholder_mapping = bool(
                raw_variables and any(k in _MAPPING_KEYS for k in raw_variables)
            )
            ...
            if has_quick_reply:
                ...
                logger.info(
                    f"Template node '{node_id}': Non-interactive template, "
                    f"passthrough to next node"
                )
                new_state = {
                    **state,
                    "current_node_id": node_id,
...
def _unassign_contact_on_flow_end(contact_id: int, flow_id: int):
    ...
    try:
        ...
        from team_inbox.models import (ActorTypeChoices, Event,
                                       EventTypeChoices, MessageEventIds)
        ...
        if (contact.assigned_to_type != AssigneeTypeChoices.CHATFLOW or 
            contact.assigned_to_id != flow_id):
            logger.info(
...
def create_api_call_node_handler(node: ChatFlowNode, edges: List[ChatFlowEdge]):
    ...
    def handler(state: FlowState) -> FlowState:
        ...
        logger.info(
            f"API node '{node_id}': {api_method} {url} "
            f"(timeout={api_timeout}s, retries={api_retry_count})"
        )
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                last_error = str(exc)
                logger.warning(
                    f"API node '{node_id}': Attempt {attempt}/{attempts} failed: {exc}"
                )
        ...
        logger.info(
            f"API node '{node_id}': Routing with '{route_key}'"
        )
...
def _send_api_error_email(
    ...
    try:
        ...
        if not recipient_emails:
            logger.warning(
                f"API error email: No active users with email for tenant {tenant.pk}"
            )
            return
...
def create_condition_node_handler(node: ChatFlowNode):
    ...
    def _evaluate_group(state: FlowState, group: dict) -> bool:
        ...
        for rule in rules:
            ...
            logger.debug(
                ...
                f"{variable}='{actual}' {operator} '{value}' → {result}"
            )
    ...
    def handler(state: FlowState) -> FlowState:
        ...
        logger.info(
            f"Executing condition node '{node_id}' with "
            f"{len(condition_groups)} group(s), {total_rules} rule(s)"
        )
        ...
        for idx, group in enumerate(condition_groups):
            ...
            logger.debug(
                f"Condition node '{node_id}': group {idx + 1} "
                f"(inner_logic={group.get('logic', 'and')}) → {result}"
            )
...
def create_delay_node_handler(node: ChatFlowNode, edges: List[ChatFlowEdge]):
    ...
    def handler(state: FlowState) -> FlowState:
        ...
        logger.info(
            f"Delay node '{node_id}': Pausing graph, "
            f"delay={total_delay_seconds}s, next_node='{next_node_id}'"
        )
...
def create_message_node_handler(node: ChatFlowNode, edges: List[ChatFlowEdge]):
    ...
    has_quick_reply = any(
        btn.get('type') == 'QUICK_REPLY' for btn in message_buttons
    )
    ...
    def handler(state: FlowState) -> FlowState:
        ...
        if resume_target and resume_target == node_id:
            ...
            if pending:
                state["user_input"] = pending
                logger.info(
                    f"Message node '{node_id}': Restored pending user input '{pending}'"
                )
        ...
        if result.get("success"):
            ...
            logger.error(
                f"Message node '{node_id}': Failed to send session message: "
                f"{result.get('error')}"
            )
        ...
        if has_quick_reply:
            ...
            logger.info(
                f"Message node '{node_id}': Has QUICK_REPLY buttons, "
...
def send_session_message(
    ...
    """
    from contacts.models import TenantContact
    from wa.models import (MessageDirection, MessageStatus, MessageType,
                           WAMessage)
    from wa.utility.data_model.gupshup.session_message_base import (
        ButtonReply, InteractiveBody, InteractiveButton,
        InteractiveButtonAction, InteractiveButtonContent,
        InteractiveButtonMessage, InteractiveFooter,
        InteractiveHeader, InteractiveOrderDetailsContent,
        InteractiveOrderDetailsMessage, OrderDetailsAction,
        OrderDetailsParameters, OrderSession, TextMessage,
        TextMessageInput)
    from wa.utility.data_model.shared.order_models import (
        OrderAmount, OrderItem, PaymentSettings)
    ...
    try:
        ...
        contact = TenantContact.objects.select_related(
            'tenant'
        ).get(id=contact_id)

        # Get the WA app for this tenant (first active app)
        from tenants.models import TenantWAApp
        wa_app = TenantWAApp.objects.select_related('waba_info').filter(
            tenant=contact.tenant,
            is_active=True
        ).first()
        ...
        quick_reply_buttons = [
            b for b in node_buttons
            if (b.get('type', 'QUICK_REPLY')).upper() in ('QUICK_REPLY', 'QUICK-REPLY')
        ]
        ...
        if is_interactive and quick_reply_buttons:
            ...
            for btn in quick_reply_buttons[:3]:  # WhatsApp max 3 buttons
                ...
                buttons.append(
                    InteractiveButton(reply=ButtonReply(id=btn_id, title=btn_title))
                )
            ...
            logger.info(
                f"Session message: interactive_button with {len(buttons)} buttons "
            ...
            for ri in raw_items:
                item_kwargs: Dict[str, Any] = {
                    ...
                    'amount': OrderAmount(**ri['amount']) if isinstance(ri.get('amount'), dict) else OrderAmount(value=0),
            ...
            order_kwargs: Dict[str, Any] = {
                'items': items,
                'subtotal': OrderAmount(**od['order']['subtotal']) if isinstance(od.get('order', {}).get('subtotal'), dict) else OrderAmount(value=0),
                'tax': OrderAmount(**od['order']['tax']) if isinstance(od.get('order', {}).get('tax'), dict) else OrderAmount(value=0),
            }
            ...
            parameters = OrderDetailsParameters(
                ...
                total_amount=OrderAmount(**od['total_amount']) if isinstance(od.get('total_amount'), dict) else OrderAmount(value=0),
            ...
            logger.info(
                f"Session message: order_details ref={ref_id} "
                f"items={len(items)} for contact {contact_id}"
            )
            ...
            logger.info(
                f"Session message: {message_type} for contact {contact_id}, "
                f"url={media_url[:80]}..."
            )
            ...
            message = TextMessage(
                to=recipient,
                text=TextMessageInput(body=message_content)
            )
            payload = message.model_dump(by_alias=True, exclude_none=True)
            ...
            message = TextMessage(
                to=recipient,
                text=TextMessageInput(body=body_text or message_content)
            )
            payload = message.model_dump(by_alias=True, exclude_none=True)
            wa_message_type = MessageType.TEXT
            stored_text = body_text or message_content
...
def create_button_router(node: ChatFlowNode, edges: List[ChatFlowEdge]):
    ...
    for edge in edges:
        if edge.source_node.id == node.id:
            ...
            if isinstance(btn_text, list):
                logger.warning(
                    f"Edge {edge.edge_id}: button_text is a list {btn_text}, "
    ...
    def router(state: FlowState) -> str:
        ...
        logger.warning(
            f"Node '{node_id}': Unknown button '{user_input}'. "
            f"Available buttons: {list(button_routes.keys())}"
        )
        return END
...
class ChatFlowExecutor:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        edges = list(ChatFlowEdge.objects.filter(flow=self.flow).select_related(
    ...
    def process_input(
        ...
        if current_state and current_state.get("is_complete"):
            logger.info(
                f"In-memory state for contact {contact_id} is complete — "
                f"checking DB for a newer active session"
            )
        ...
        if user_input == "__DELAY_CONTINUE__" and resume_from:
            ...
            logger.info(
                f"Delay resume for contact {contact_id}: "
                f"skip-to node '{resume_from}'"
            )
    ...
    def _schedule_pending_delay(self, contact_id: int) -> None:
        ...
        if not next_node_id:
            logger.error(
                f"Delay info missing next_node_id for contact {contact_id}, "
                f"flow {self.flow_id}"
            )
            return
        ...
        try:
            ...
            is_eager = getattr(settings, 'CELERY_TASK_ALWAYS_EAGER', False) or \
            ...
            if is_eager:
                ...
                def _fire_after_delay():
                    ...
                    import time
                    logger.info(
                        f"Eager-mode delay: sleeping {delay_seconds}s for "
                        f"flow={flow_id}, contact={contact_id}"
                    )
                    time.sleep(delay_seconds)
                    continue_flow_after_delay(flow_id, contact_id, next_node_id,
            ...
            logger.exception(
                f"Failed to schedule delay for contact {contact_id}: {e}"
            )
    ...
    def display_graph(self, format: str = "mermaid"):
        ...
        if format == "mermaid":
            try:
                ...
                display(Markdown(f"### ChatFlow: {self.flow.name} (ID: {self.flow_id})\n\n```mermaid\n{mermaid_str}\n```"))
                return mermaid_str
    ...
    def get_graph_info(self) -> dict:
        ...
        nodes = list(ChatFlowNode.objects.filter(flow=self.flow).values(
            'id', 'node_id', 'node_type', 'template__element_name', 'position_x', 'position_y'
        ))
        
        edges = list(ChatFlowEdge.objects.filter(flow=self.flow).values(
            'id', 'edge_id', 'source_node__node_id', 'target_node__node_id', 
    ...
    def debug_session(self, contact_id: int) -> dict:
        ...
        if state:
            ...
            if current_node_id:
                try:
                    node = ChatFlowNode.objects.select_related('template').get(
                        flow=self.flow,
                    ...
                    result['current_node_info'] = {
                        ...
                        'outgoing_edges': list(node.outgoing_edges.values(
                            'button_text', 'target_node__node_id'
                        ))
                    }
                except ChatFlowNode.DoesNotExist:
        ...
        # Get DB session
        try:
            db_session = UserChatFlowSession.objects.get(
                contact_id=contact_id,
                flow=self.flow,
                is_active=True
            )
...
def debug_chatflow(flow_id: int, contact_id: int = None) -> dict:
    ...
    if contact_id and result.get("session_info"):
        ...
        if "error" in result["session_info"]:
            ...
            if mem:
                print(f"  Memory State: current_node={mem.get('current_node_id')}, awaiting={mem.get('awaiting_input')}")
            else:
                print(f"  Memory State: NOT IN MEMORY (will load from DB)")
