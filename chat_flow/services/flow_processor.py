...
from ..constants import NON_TEMPLATE_NODE_TYPES, is_multi_output_node, is_passthrough_node
...
class ButtonElement(InteractionElement):
    ...
    def extract_from_node_data(cls, node_data: Dict, node_id: str) -> List[Dict]:
        ...
        raw_buttons = node_data.get("buttons", []) or node_data.get("selected_template", {}).get("buttons", []) or []
            ...
            buttons.append(
                {
                    ...
                    **{
                        ...
                        if k not in ["id", "text", "type", "index", "template_button_index", "element_type"]
                    },
                }
            )
...
class InteractionElementExtractor:
    ...
    @classmethod
    def extract_all(
        cls, template: Optional[WATemplate], node_data: Dict, node_id: str, element_types: Optional[List[str]] = None
    ) -> Dict[str, List[Dict]]:
        """
    ...
    @classmethod
    def get_all_elements_flat(cls, template: Optional[WATemplate], node_data: Dict, node_id: str) -> List[Dict]:
        """
    ...
    def find_element_by_text(
        cls, template: Optional[WATemplate], node_data: Dict, node_id: str, text: str
    ) -> Optional[Dict]:
    ...
    def find_element_by_id(
        cls, template: Optional[WATemplate], node_data: Dict, node_id: str, element_id: str
    ) -> Optional[Dict]:
...
class ChatFlowProcessor:
    ...
    def create_flow_from_reactflow(
        ...
        with transaction.atomic():
            ...
            start_node = next((n for n in validated_flow.nodes if n.type == "start" or n.data.is_start_node), None)

            if start_node and validated_flow.edges:
                # Find edge from start node
                start_edge = next((e for e in validated_flow.edges if e.source == start_node.id), None)
                if start_edge:
                    # Find target node of start edge
                    first_template_node = next(
                        (n for n in validated_flow.nodes if n.id == start_edge.target and n.type == "template"), None
                    )

            # Fallback: find first template node in the list
            if not first_template_node:
                first_template_node = next((n for n in validated_flow.nodes if n.type == "template"), None)
            ...
            if start_template_id:
                try:
                    start_template = WATemplate.objects.get(
                        id=start_template_id, wa_app__tenant=tenant, status="APPROVED"
                    )
            ...
            for node_obj in validated_flow.nodes:
                node_obj = ChatFlowProcessor._create_node_from_validated_data(flow, node_obj, tenant, nodes_map)

            # Step 6: Create edges using validated edge objects
            for edge_obj in validated_flow.edges:
                ChatFlowProcessor._create_edge_from_validated_data(flow, edge_obj, nodes_map)
    ...
    def update_flow_from_reactflow(flow: ChatFlow, flow_data: Dict, tenant) -> ChatFlow:
        ...
        with transaction.atomic():
            ...
            if not structure_changed:
                ...
                incoming_nodes_by_id = {n["id"]: n.get("data", {}) for n in new_flow_data.get("nodes", [])}
            ...
            if validated_flow.nodes:
                ...
                if start_template_id:
                    try:
                        start_template = WATemplate.objects.get(id=start_template_id, status="APPROVED")
            ...
            nodes_map = {}
            for node_obj in validated_flow.nodes:
                ChatFlowProcessor._create_node_from_validated_data(flow, node_obj, tenant, nodes_map)
    ...
    def _compute_flow_structure_hash(flow_data: Dict) -> str:
        ...
        for node in flow_data.get("nodes", []):
            ...
            for btn in node_data.get("buttons", []):
                buttons.append({"text": btn.get("text"), "type": btn.get("type")})
        ...
        structure = {"nodes": nodes_structure, "edges": edges_structure}
    ...
    def validate_flow_rules(
        flow_data: Dict, skip_db_checks: bool = False, return_warnings: bool = False
    ) -> ValidationResult:
        """
    ...
    def _create_node_from_validated_data(
        flow: ChatFlow, node_obj: ReactFlowNode, tenant, nodes_map: Dict[str, ChatFlowNode]
    ) -> ChatFlowNode:
        """
        ...
        if template_id:
            try:
                template = WATemplate.objects.get(id=template_id, wa_app__tenant=tenant, status="APPROVED")
            except WATemplate.DoesNotExist:
        ...
        interaction_elements = InteractionElementExtractor.extract_all(
            template=template, node_data=node_data_dict, node_id=node_obj.id
        )
        ...
        if interaction_elements.get("button") and template.buttons:
            ChatFlowProcessor._validate_interaction_elements_with_template(
                interaction_elements["button"], template, element_type="button"
            )
    ...
    def _validate_interaction_elements_with_template(
        elements: List[Dict], template: WATemplate, element_type: str = "button"
    ) -> bool:
        """
        ...
        if element_type == "button":
            ...
            for element in elements:
                ...
                if element_text and element_text not in template_button_texts:
                    raise ValueError(f"Button '{element_text}' not found in template '{template.element_name}'")
    ...
    def _create_edge_from_validated_data(
        flow: ChatFlow, edge_obj: ReactFlowEdge, nodes_map: Dict[str, ChatFlowNode]
    ) -> ChatFlowEdge:
        """
        ...
        # Method 3: sourceHandle like 'button-0', 'card-1', 'list_item-2'
            ...
            element = ChatFlowProcessor._resolve_element_from_source_handle(
                source_node=source_node, source_handle=edge_obj.sourceHandle
            )
            if element:
                element_id = element.get("id")
                element_text = element.get("text")
    ...
    @staticmethod
    def _resolve_element_from_source_handle(source_node: ChatFlowNode, source_handle: str) -> Optional[Dict]:
        """
    ...
    def _validate_button_exists(template: WATemplate, button_text: str) -> bool:
        ...
        button_exists = any(button.get("text") == button_text for button in template_buttons)

        if not button_exists:
            raise ValueError(f"Button '{button_text}' does not exist in template '{template.element_name}'")
    ...
    def _find_matching_template(node_buttons: List[Dict], tenant) -> Optional[WATemplate]:
        ...
        approved_templates = WATemplate.objects.filter(wa_app__tenant=tenant, status="APPROVED", buttons__isnull=False)
        ...
        for node_button in node_buttons:
            ...
            if template_button_index is not None:
                if template_button_index >= len(template_buttons):
                    raise ValueError(
                        f"Invalid template_button_index {template_button_index} for button '{button_text}'"
                    )
...
class ReactFlowConverter:
    ...
    def flow_to_reactflow(flow: ChatFlow) -> Dict:
        ...
        for node in flow.nodes.all():
            ...
            try:
                ...
                ReactFlowViewport(x=node.position_x, y=node.position_y)
        ...
        for edge in flow.edges.all():
            ...
            source_node_data = next((n for n in nodes if n["id"] == edge.source_node.node_id), {})
            ...
            edge_data_dict = {"button_text": edge.button_text, "button_type": edge.button_type, **edge.edge_data}
        ...
        flow_data = {"nodes": nodes, "edges": edges, "viewport": viewport_dict}
    ...
    def migrate_legacy_flow_data(flow: ChatFlow) -> Dict:
        ...
        for node_data in nodes:
            ...
            try:
                ...
                validated_node = ReactFlowNode(
                    id=node_id, type=node_type, position=validated_position, data=validated_node_data
                )
                ...
                migrated_nodes.append(
                    {
                        ...
                        "type": node_type,
                        "position": position,
                        "data": {**data, "buttons": validated_buttons},
                    }
                )
        ...
        for edge_data in edges:
            ...
            try:
                validated_edge_data = EdgeData(**edge_data_dict)
                validated_edge = ReactFlowEdge(id=edge_id, source=source, target=target, data=validated_edge_data)
                migrated_edges.append(validated_edge.model_dump())

            except Exception as e:
                # Fallback to original structure
                print(f"Warning: Edge migration failed for {edge_id}: {e}")
                migrated_edges.append({"id": edge_id, "source": source, "target": target, "data": edge_data_dict})
        ...
        migrated_data = {"nodes": migrated_nodes, "edges": migrated_edges, "viewport": viewport_dict}
