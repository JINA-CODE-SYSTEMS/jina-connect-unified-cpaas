...
class ChatFlowAdmin(admin.ModelAdmin):
    list_display = (
        'name', 'tenant', 'is_active', 'node_count', 'edge_count',
        'health_badge', 'session_count', 'active_session_count', 'created_at', 'updated_at',
    )
    list_filter = ('is_active', 'tenant', 'created_at')
    search_fields = ('name', 'description', 'start_template__element_name')
    readonly_fields = (
        'created_at', 'updated_at', 'health_report', 'flow_graph_preview',
        'node_count', 'edge_count', 'session_count', 'active_session_count', 'test_session_link',
    )
    ...
    def health_report(self, obj):
        ...
        if result.warnings:
            summary_parts.append(f'<span style="color:#d97706;font-weight:700;">{len(result.warnings)} warning(s)</span>')
        if result.info:
    ...
    def flow_graph_preview(self, obj):
        ...
        for n in nodes:
            ...
            icon = {
                'start': '🟢', 'end': '🔴', 'template': '📋', 'message': '💬',
                'condition': '🔀', 'delay': '⏱️', 'api': '🌐',
                'handoff': '🤝', 'action': '⚡',
            }.get(ntype, '📦')
    ...
    def deactivate_flows(self, request, queryset):
        ...
        for flow in queryset:
            ended = UserChatFlowSession.objects.filter(
                flow=flow, is_active=True
            ).update(is_active=False, ended_at=now, cancellation_reason='Bulk deactivated via admin')
    ...
    def start_test_session(self, request, queryset):
        ...
        flow = queryset.first()
        return redirect(
            reverse('admin:chatflow_start_test_session', args=[flow.pk])
        )
    ...
    def start_test_session_view(self, request, flow_id):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect(
                    reverse('admin:chat_flow_chatflow_change', args=[flow.pk])
                )
        else:
...
class ChatFlowNodeAdmin(admin.ModelAdmin):
    list_display = (
        'node_id_short', 'flow', 'node_type_badge', 'template_name',
        'outgoing_edge_count', 'incoming_edge_count', 'position_x', 'position_y',
    )
    ...
    def node_type_badge(self, obj):
        colours = {
            'start': '#16a34a', 'end': '#dc2626', 'template': '#7c3aed',
            'message': '#2563eb', 'condition': '#d97706', 'delay': '#6b7280',
            'api': '#0891b2', 'handoff': '#db2777', 'action': '#ea580c',
        }
...
class ChatFlowEdgeAdmin(admin.ModelAdmin):
    list_display = (
        'edge_id_short', 'flow', 'source_label', 'target_label',
        'button_text', 'routing_type',
    )
    ...
    def routing_type(self, obj):
        ...
        return format_html(
            '<span style="color:{};">{} {}</span>', c, icon, el_type,
        )
...
def _build_session_debug_context(session):
    ...
    for bc in chatflow_broadcasts:
        bm = BroadcastMessage.objects.filter(
            broadcast=bc, contact=session.contact,
        ).first()
...
class UserChatFlowAdmin(admin.ModelAdmin):
    list_display = (
        'id', 'contact', 'flow', 'tenant', 'current_node_id',
        'status_badge', 'inspector_link', 'started_at', 'ended_at',
    )
    list_filter = ('tenant', 'flow', 'is_active', 'is_complete', 'started_at')
    search_fields = ('contact__phone', 'flow__name', 'current_node_id')
    readonly_fields = ('started_at', 'session_inspector',)
    ...
    def inspector_link(self, obj):
        ...
        return format_html(
            '<a href="{}" style="color:#7c3aed;font-weight:600;">🔍 Inspect</a>', url
        )
    ...
    def session_inspector(self, obj):
        ...
        for bc in broadcasts:
            ...
            bs_colours = {
                'DRAFT': '#6b7280', 'QUEUED': '#d97706', 'SCHEDULED': '#d97706',
                'SENDING': '#2563eb', 'SENT': '#16a34a', 'PARTIALLY_SENT': '#d97706',
            ...
            ms = bc['msg_status']
            ms_colours = {
                'PENDING': '#6b7280', 'QUEUED': '#d97706', 'SENDING': '#2563eb',
                'SENT': '#16a34a', 'DELIVERED': '#16a34a', 'READ': '#059669',
    ...
    def session_inspector_view(self, request, session_id):
        session = UserChatFlowSession.objects.select_related(
            'flow', 'contact', 'tenant'
        ).get(pk=session_id)
    ...
    def reactivate_sessions(self, request, queryset):
        updated = queryset.filter(is_active=False, is_complete=False).update(
