...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--id',
            type=int,
            help='ID of the IncomingTemplateWebHookDump row to process'
        )
        parser.add_argument(
            '--all-unprocessed',
            action='store_true',
            help='Process all unprocessed webhooks'
        )
        parser.add_argument(
            '--skip-email',
            action='store_true',
    ...
    def process_webhook(self, webhook: IncomingTemplateWebHookDump, skip_email: bool = False, dry_run: bool = False):
        ...
        template = WATemplate.objects.filter(
            template_id=processed_data.gs_template_id,
            wa_app=wa_app
        ).first()
        
        if not template:
            self.stdout.write(self.style.WARNING(f'  Template not found for gs_template_id: {processed_data.gs_template_id}'))
        ...
        if processed_data.event == "CATEGORY_UPDATE":
            ...
            if dry_run:
                self.stdout.write(self.style.WARNING(f'  [DRY RUN] Would update category: {template.category} -> {processed_data.new_category}'))
                ...
                self.stdout.write(self.style.SUCCESS(f'  Updated category: {old_category} -> {processed_data.new_category}'))
        ...
        # Handle status update
            ...
            if new_status:
                if dry_run:
                    self.stdout.write(self.style.WARNING(f'  [DRY RUN] Would update status: {template.status} -> {new_status}'))
                else:
                    old_status = template.status
                    ...
                    if not skip_email and processed_data.event.upper() in ['APPROVED', 'REJECTED', 'FAILED', 'DISABLED']:
                        self.stdout.write('  Sending email notification...')
