    ...
    def handle(self, *args, **options):
        ...
        for template in templates:
            ...
            for i, card in enumerate(cards):
                ...
                self.stdout.write(f"  [{template.element_name}] Card {i}: headerType missing → set to {detected}")
            ...
            if apply:
                ...
                self.stdout.write(self.style.SUCCESS(f"  ✓ Saved {template.element_name} ({template.pk})"))
            else:
                self.stdout.write(f"  (dry-run) Would save {template.element_name} ({template.pk})")
