...
class WATemplateFilter(django_filters.FilterSet):
    ...
    has_buttons = django_filters.BooleanFilter(method="filter_has_buttons", label="Has any buttons")
    has_quick_reply_buttons = django_filters.BooleanFilter(
        method="filter_has_quick_reply_buttons", label="Has QUICK_REPLY buttons"
    )
    has_call_button = django_filters.BooleanFilter(
        method="filter_has_call_button", label="Has PHONE_NUMBER (call) buttons"
    )
    has_url_button = django_filters.BooleanFilter(method="filter_has_url_button", label="Has URL buttons")
    has_otp_button = django_filters.BooleanFilter(method="filter_has_otp_button", label="Has OTP buttons")
    ...
    def filter_has_buttons(self, queryset, name, value):
        ...
        if value:
            ...
            return queryset.exclude(Q(buttons__isnull=True) | Q(buttons=[]))
        else:
            # No buttons: buttons is null or empty
            return queryset.filter(Q(buttons__isnull=True) | Q(buttons=[]))
    ...
    def filter_has_quick_reply_buttons(self, queryset, name, value):
        ...
        has_quick_reply = queryset.filter(buttons__contains=[{"type": "QUICK_REPLY"}])
    ...
    def filter_has_call_button(self, queryset, name, value):
        ...
        has_call = queryset.filter(buttons__contains=[{"type": "PHONE_NUMBER"}])
    ...
    def filter_has_url_button(self, queryset, name, value):
        ...
        has_url = queryset.filter(buttons__contains=[{"type": "URL"}])
    ...
    def filter_has_otp_button(self, queryset, name, value):
        ...
        has_otp = queryset.filter(buttons__contains=[{"type": "OTP"}])
