...
class WAMessageViewSet(BaseTenantModelViewSet):
    ...
    @swagger_auto_schema(
        ...
        responses={
            200: openapi.Response(description="List of messages", schema=WAMessageListSerializer(many=True)),
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                "contact_id", openapi.IN_QUERY, description="Contact ID", type=openapi.TYPE_INTEGER, required=True
            ),
            openapi.Parameter(
                "wa_app",
                openapi.IN_QUERY,
        ...
        responses={
            200: openapi.Response(description="Conversation messages", schema=WAMessageSerializer(many=True)),
    ...
    def conversation(self, request):
        ...
        if not contact_id or not wa_app_id:
            return Response({"error": "Both contact_id and wa_app are required"}, status=status.HTTP_400_BAD_REQUEST)

        queryset = self.get_queryset().filter(contact_id=contact_id, wa_app_id=wa_app_id).order_by("created_at")
