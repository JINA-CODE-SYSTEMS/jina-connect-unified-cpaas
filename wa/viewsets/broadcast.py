...
class WABroadcastViewSet(BroadcastViewSet):
    ...
    def get_sending_quota(self, request):
        ...
        if not wa_app:
            return Response(
                {"error": "No WhatsApp app found. Please provide valid wa_app_id."}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def get_charge_breakdown(self, request):
        ...
        """
        from broadcast.services.charge_breakdown import ASYNC_CONTACT_THRESHOLD, ChargeBreakdownService
        ...
        if contact_ids:
            ...
            try:
                contact_count = Broadcast.objects.get(id=broadcast_id, tenant=request.user.tenant).recipients.count()
