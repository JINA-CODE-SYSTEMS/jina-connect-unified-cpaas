...
class WABroadcastSerializer(BroadcastSerializer):
    ...
    catalog_id = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, help_text="Catalog ID for catalog-type messages"
    )
    ...
    def validate(self, attrs):
        ...
        if not wa_app and instance:
            if (
                instance.template_number
                and hasattr(instance.template_number, "gupshup_template")
                and instance.template_number.gupshup_template
            ):
        ...
        validation_result = quota_service.validate_broadcast(
            recipient_phones=phone_list, scheduled_time=scheduled_time, exclude_broadcast_id=broadcast_id
        )
    ...
    def _resolve_select_all_phones(self, attrs):
        ...
        qs = TenantContact.objects.filter(tenant=tenant, phone__isnull=False).exclude(phone="")
        if tag_filter:
    ...
    def _check_balance_for_broadcast(self, wa_app, recipients, phone_list):
        ...
        if available_balance < estimated_cost:
            raise serializers.ValidationError(
                {
                    "balance": (
                        f"Insufficient balance. Estimated cost: {estimated_cost}, Available: {available_balance}."
                    ),
                    "balance_details": {
