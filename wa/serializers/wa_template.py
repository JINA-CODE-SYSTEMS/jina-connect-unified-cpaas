...
class WATemplateV2ListSerializer(BaseSerializer):
    ...
    tenant_media_url = serializers.SerializerMethodField(
        read_only=True,
        help_text="Local file URL for header media (IMAGE/VIDEO/DOCUMENT templates)"
    )
    tenant_media_id = serializers.UUIDField(
        source='tenant_media.id',
        read_only=True,
        help_text="TenantMedia ID for header media"
    )
...
class WATemplateV2Serializer(BaseSerializer):
    ...
    template_type_display = serializers.CharField(
        ...
        read_only=True,
        help_text="Human-readable template type"
    )
    wa_app_phone = serializers.CharField(
        source='wa_app.phone_number',
        read_only=True,
        help_text="Phone number of the associated WA App"
    )
    tenant_media_url = serializers.SerializerMethodField(
        read_only=True,
        help_text="Local file URL for header media (IMAGE/VIDEO/DOCUMENT templates)"
    ...
    card_media_details = serializers.SerializerMethodField(
        read_only=True,
        help_text="Local file URLs for carousel card media, keyed by card_index"
    )
    ...
    class Meta:
        ...
        extra_kwargs = {
            'element_name': {
                'help_text': 'Unique template name (lowercase, underscores only)'
            },
            'language_code': {
                'help_text': 'Template language code (e.g., en, en_US)'
            },
            'content': {
                'help_text': 'Body text with placeholders {{name}}, {{order_id}}'
            },
            'buttons': {
                'help_text': 'Array of button objects (max 3)'
            },
            'cards': {
                'help_text': 'Carousel card objects (max 10)'
            },
        }
        swagger_schema_fields = {
            "type": openapi.TYPE_OBJECT,
    ...
    def validate_element_name(self, value):
        ...
        if not re.match(r'^[a-z][a-z0-9_]*$', value):
            raise serializers.ValidationError(
                "Element name must start with a letter and contain only "
    ...
    def validate(self, data):
        ...
        if template_type and template_type not in ('CAROUSEL', 'CATALOG'):
            if not content or not content.strip():
                raise serializers.ValidationError({
                    'content': 'Body text is required for non-carousel templates.'
                })
        ...
        if category == 'AUTHENTICATION':
            if not has_buttons:
                raise serializers.ValidationError({
                    'buttons': 'OTP buttons are mandatory for AUTHENTICATION templates'
                })
            ...
            if non_otp:
                raise serializers.ValidationError({
                    'buttons': 'AUTHENTICATION templates can only have OTP buttons'
                })
