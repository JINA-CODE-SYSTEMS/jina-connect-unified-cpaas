...
class WAAppSerializer(BaseSerializer):
    ...
    phone_number = serializers.CharField(
        source='wa_number',
        help_text='WhatsApp phone number with country code (e.g., +919876543210)'
    )
    bsp_display = serializers.CharField(
        source='get_bsp_display',
        read_only=True,
        help_text="Human-readable BSP name"
    )
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            'bsp': {
                'help_text': 'Business Solution Provider (META, GUPSHUP, etc.)'
            },
        }
        swagger_schema_fields = {
            "type": openapi.TYPE_OBJECT,
...
class WAAppSafeSerializer(BaseSerializer):
    ...
    phone_number = serializers.CharField(
        source='wa_number',
        help_text='WhatsApp phone number with country code'
    )
    bsp_display = serializers.CharField(
        source='get_bsp_display',
        read_only=True,
    ...
    def validate(self, data):
        ...
        if bsp == 'META':
            ...
            if not data.get('phone_number_id'):
                raise serializers.ValidationError({
                    'phone_number_id': 'Phone Number ID is required for META BSP'
                })
