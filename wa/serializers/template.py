...
class WATemplateSerializer(BaseSerializer):
    ...
    buttons = TemplateButtonsField(
        required=False, 
        allow_null=True,
        help_text="List of template buttons (maximum 3 allowed)"
    )
    
    cards = TemplateCardsField(
        required=False,
        allow_null=True,
        help_text="List of template cards for carousel templates (maximum 10 allowed)"
    )

    tag_names = serializers.SerializerMethodField(
        help_text="List of tag names associated with the template"
    )
    template_media_url = serializers.SerializerMethodField(
        help_text="URL of the media associated with the template"
    )
    cards_media_urls = serializers.SerializerMethodField(
        help_text="List of media URLs for each card in the template"
    )
    ...
    class Meta:
        ...
        extra_kwargs = {
            'element_name': {
                'help_text': 'Unique element name (letters, numbers, underscores, hyphens only)'
            },
            'language_code': {
                'help_text': 'Template language code (e.g., en_US, hi)'
            },
            'category': {
                'help_text': 'Template category determines usage restrictions'
            },
            'template_type': {
                'help_text': 'Type of template content'
            },
            'content': {
                'help_text': 'Template body text. Use {{name}}, {{code}}, etc. for dynamic parameters'
            },
