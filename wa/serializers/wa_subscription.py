...
class WASubscriptionV2Serializer(BaseSerializer):
    ...
    status_display = serializers.CharField(
        source="get_status_display", read_only=True, help_text="Human-readable subscription status"
    )
    wa_app_name = serializers.CharField(source="wa_app.name", read_only=True, help_text="Name of the associated WA App")
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            "webhook_url": {"help_text": "URL to receive webhook events (must be HTTPS)"},
            "event_types": {"help_text": 'List of event types to subscribe to (e.g., ["message", "status"])'},
        }
        swagger_schema_fields = {
            "type": openapi.TYPE_OBJECT,
    ...
    def validate_webhook_url(self, value):
        ...
        if not value.startswith("https://"):
            raise serializers.ValidationError("Webhook URL must use HTTPS for security")
    ...
    def validate_event_types(self, value):
        ...
        if invalid:
            raise serializers.ValidationError(
                f"Invalid event types: {', '.join(invalid)}. Valid types: {', '.join(valid_types)}"
            )
