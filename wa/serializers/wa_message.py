...
from wa.models import WAMessage, WAApp
...
class WAMessageCreateSerializer(BaseSerializer):
    ...
    gupshup_model = serializers.CharField(
        required=False, write_only=True,
        help_text="(Legacy) WA App ID string — mapped to wa_app"
    )
    payload = serializers.JSONField(
        required=False, write_only=True,
        help_text="(Legacy) Raw WhatsApp Cloud API payload"
    )
    ...
    body_text = serializers.CharField(
        required=False, write_only=True,
        help_text="Body text for interactive messages"
    )
    header_text = serializers.CharField(
        required=False, write_only=True,
        help_text="Header text for interactive messages (optional)"
    )
    footer_text = serializers.CharField(
        required=False, write_only=True,
        help_text="Footer text for interactive messages (optional)"
    )
    ...
    class Meta:
        ...
        extra_kwargs = {
            'wa_app': {
                'required': False,
                'help_text': 'WA App to send message from'
            },
            'contact': {
                'required': False,
                'help_text': 'Contact to send message to'
            },
            'message_type': {
                'required': False,
                'help_text': 'Type of message (TEXT, TEMPLATE, IMAGE, etc.)'
            },
            'text': {
                'required': False,
                'help_text': 'Message text for TEXT type messages'
            },
            'template': {
                'required': False,
                'help_text': 'Template ID for TEMPLATE type messages'
            },
            'template_params': {
                'help_text': 'Parameters for template placeholders'
            },
        }
    ...
    def to_internal_value(self, data):
        ...
        logger.info(
            ...
            gupshup_model_val, wa_app.pk, mutable.get('message_type')
        )
    ...
    def validate(self, data):
        ...
        if message_type == 'TEXT' and not data.get('text'):
            raise serializers.ValidationError({
                'text': 'Text is required for TEXT message type'
            })
        
        # TEMPLATE messages require template
        if message_type == 'TEMPLATE' and not data.get('template'):
            raise serializers.ValidationError({
                'template': 'Template is required for TEMPLATE message type'
            })
        ...
        if message_type == 'INTERACTIVE':
            ...
            if not data.get('body_text'):
                raise serializers.ValidationError({
                    'body_text': 'body_text is required for INTERACTIVE message type'
                })
    ...
    def _build_raw_payload(self, data):
        ...
        if msg_type == 'text':
            ...
            from wa.utility.data_model.gupshup.session_message_base import (
                InteractiveOrderDetailsMessage, InteractiveOrderDetailsContent,
                OrderDetailsAction, OrderDetailsParameters,
                InteractiveBody, InteractiveHeader, InteractiveFooter,
            )
            ...
            message = InteractiveOrderDetailsMessage(
                ...
                interactive=InteractiveOrderDetailsContent(
                    ...
                    action=OrderDetailsAction(
                        parameters=OrderDetailsParameters(**order_details_data)
                    ),
                )
            )
...
class WAMessageSerializer(BaseSerializer):
    ...
    direction_display = serializers.CharField(
        ...
        read_only=True,
        help_text="Human-readable direction (Inbound/Outbound)"
    )
    ...
    message_type_display = serializers.CharField(
        ...
        read_only=True,
        help_text="Human-readable message type"
    )
    ...
    template_name = serializers.CharField(
        source='template.element_name',
        read_only=True,
        help_text="Name of the template used (if any)"
    )
