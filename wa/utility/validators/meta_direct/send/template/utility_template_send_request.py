"""
Utility Template Send Request Validator for META Direct API

This module provides Pydantic validators for META's WhatsApp Business API
utility template message sending requests.
...
class UtilityTemplateSendRequestValidator(BaseModel):
    """
    Validator for META Direct API utility template send request.
    ...
    Example usage:
        ...
        ...                 {"type": "text", "parameter_name": "order_id", "text": "ORD-12345"},
...
def validate_utility_template_send(data: dict) -> UtilityTemplateSendRequestValidator:
    """
    Validate a utility template send request dictionary.

    Args:
        data: Dictionary containing request data

    Returns:
        UtilityTemplateSendRequestValidator: Validated request object

    Raises:
        ValidationError: If request data is invalid
    """
    return UtilityTemplateSendRequestValidator(**data)


def parse_and_validate_utility_template_send(
    json_str: str,
) -> UtilityTemplateSendRequestValidator:
    """
    Parse JSON string and validate as utility template send request.
    ...
    return validate_utility_template_send(data)
