...
from wa.utility.data_model.gupshup.template_button_input import TemplateButtonsInput, parse_template_buttons
...
class AuthTemplateValidator(BaseTemplateValidator):
    ...
    def validate_buttons(cls, v):
        ...
        if v is None or v == []:
            raise ValueError(
                "OTP buttons are mandatory for AUTHENTICATION category templates. At least one OTP button is required."
            )
    ...
    def _validate_otp_buttons_only(cls, buttons):
        ...
        """
        from wa.utility.data_model.gupshup.template_button_input import ButtonType
        ...
        for i, button in enumerate(buttons):
            ...
            if not button.otp_type:
                raise ValueError(f"Button {i + 1}: OTP button must specify otp_type (COPY_CODE or ONE_TAP)")

            # Validate ONE_TAP specific requirements
            if button.otp_type == "ONE_TAP":
                if not button.package_name:
                    raise ValueError(f"Button {i + 1}: ONE_TAP OTP button requires package_name")
                if not button.signature_hash:
                    raise ValueError(f"Button {i + 1}: ONE_TAP OTP button requires signature_hash")
