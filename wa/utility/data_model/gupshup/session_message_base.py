...
class SessionMessageBase(BaseModel):
    ...
    messaging_product: Literal["whatsapp"] = Field("whatsapp", description="Messaging product, fixed to 'whatsapp'")
    recipient_type: Literal["individual"] = Field("individual", description="Recipient type, fixed to 'individual'")
...
class InteractiveHeader(BaseModel):
    ...
    document: Optional[dict] = Field(
        None, description="Document object with 'id' or 'link' (required if type=document)"
    )
...
class InteractiveButtonAction(BaseModel):
    ...
    buttons: List[InteractiveButton] = Field(..., description="List of buttons (max 3)", min_length=1, max_length=3)
...
class ListSection(BaseModel):
    ...
    rows: List[ListRow] = Field(
        ..., description="List of rows in this section (max 10 rows total across all sections)", min_length=1
    )
...
class InteractiveListAction(BaseModel):
    ...
    sections: List[ListSection] = Field(
        ..., description="List of sections (max 10 sections)", min_length=1, max_length=10
    )
...
class OrderSession(BaseModel):
    ...
    tax: OrderAmount = Field(..., description="Tax amount")
    shipping: Optional[OrderAmount] = Field(None, description="Shipping cost (optional)")
    discount: Optional[OrderAmount] = Field(None, description="Discount amount (optional)")
...
class InteractiveOrderDetailsContent(BaseModel):
    ...
    header: Optional[InteractiveHeader] = Field(None, description="Optional header (text or image only)")
    footer: Optional[InteractiveFooter] = Field(None, description="Optional footer")
