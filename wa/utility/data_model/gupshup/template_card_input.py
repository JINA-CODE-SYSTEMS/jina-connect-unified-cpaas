...
class TemplateCardButton(BaseModel):
    ...
    url: Optional[str] = Field(
        None, description="URL for URL type buttons, can contain {{1}}, {{2}} etc for dynamic content"
    )
...
class TemplateCardInput(BaseModel):
    ...
    headerType: HeaderType = Field(
        default=HeaderType.IMAGE, description="Type of header media (defaults to IMAGE for carousel cards)"
    )
    ...
    sampleText: Optional[str] = Field(
        None,
        description="Sample text showing how dynamic content would appear. Auto-derived from body if not provided.",
    )
    ...
    def validate_media(self):
        ...
        if self.headerType in [HeaderType.IMAGE, HeaderType.VIDEO, HeaderType.DOCUMENT]:
            if not self.mediaUrl and not self.mediaId and not self.exampleMedia:
                raise ValueError(
                    f"Either mediaUrl, mediaId, or exampleMedia is required for {self.headerType} header type"
                )
        return self
