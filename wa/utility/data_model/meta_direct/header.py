...
class HeaderTextExample(BaseModel):
    ...
    header_text: List[str] = Field(..., description="Example values for header text parameters")
...
class HeaderComponent(BaseModel):
    ...
    text: Optional[str] = Field(None, description="Header text (required for TEXT format)")
    ...
    def validate_header(self):
        if self.format == HeaderFormat.TEXT:
            ...
            if re.search(r"\{\{[^}]+\}\}", self.text) and not self.example:
                raise ValueError("Example is required when header text contains parameters")
        elif self.format in [
            HeaderFormat.IMAGE,
            HeaderFormat.VIDEO,
            ...
            if not self.example or not isinstance(self.example, HeaderHandleExample):
                raise ValueError(f"header_handle example is required for {self.format.value} header format")
        return self
