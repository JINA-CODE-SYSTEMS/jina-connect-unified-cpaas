...
class MetaMediaAPI(WAAPI):
    ...
    def upload_media(self, file_path: str, mime_type: str = None) -> dict:
        ...
        if not mime_type:
            ...
            if not mime_type:
                raise ValueError(f"Cannot detect MIME type for '{file_path}'. Please provide mime_type explicitly.")
        ...
        if not validate_media_type(mime_type):
            raise ValueError(
                f"Unsupported MIME type: '{mime_type}'. Supported types: {list(ALL_SUPPORTED_MEDIA.keys())}"
            )
    ...
    def upload_media_from_file_object(
        ...
        if not mime_type:
            ...
            if not mime_type:
                raise ValueError(f"Cannot detect MIME type for '{filename}'. Please provide mime_type explicitly.")
    ...
    def download_media(self, media_url: str) -> bytes:
        ...
        if response.status_code not in [200, 201]:
            raise Exception(
                f"Media download failed with status {response.status_code}. Response: {response.text[:500]}"
            )
            ...
            for chunk in response.iter_content(chunk_size=8192):
                if chunk:
                    ...
                    if total_size > MAX_DOWNLOAD_SIZE:
                        raise ValueError(f"Download exceeds max size ({MAX_DOWNLOAD_SIZE:,} bytes). Aborting.")
                    f.write(chunk)
    ...
    def upload_and_get_handle(
        self, file_path: str = None, file_obj=None, filename: str = None, mime_type: str = None
    ) -> str:
        """
    ...
    def create_resumable_upload_session(self, app_id: str, file_length: int, file_type: str) -> str:
        """
        ...
        if not session_id:
            raise Exception(f"Resumable upload session creation returned no ID: {result}")
        return session_id
    ...
    def upload_file_to_session(self, session_id: str, file_bytes: bytes) -> str:
        ...
        handle = result.get("h")
        if not handle:
            raise Exception(f"Resumable upload returned no file handle: {result}")
        return handle

    def upload_media_for_template(self, app_id: str, file_obj, filename: str, mime_type: str = None) -> str:
        """
        ...
        if not mime_type:
            ...
            if not mime_type:
                raise ValueError(f"Cannot detect MIME type for '{filename}'. Provide mime_type explicitly.")
