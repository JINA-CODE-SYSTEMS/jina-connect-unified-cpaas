    ...
    def get_bsp_id_for_template(
        ...
        for attempt in range(max_retries):
            ...
            if template_data:
                ...
                if bsp_id:
                    logger.info(
                        f"[{self.bsp_type.value}] Found BSP ID '{bsp_id}' "
                        f"for template '{element_name}'"
                    )
                    return str(bsp_id)
        ...
        logger.warning(
            f"[{self.bsp_type.value}] Could not find BSP ID for template "
            f"'{element_name}' after {max_retries} attempts"
        )
        return None
    
    def sync_and_get_bsp_id(
        self, 
        element_name: str,
