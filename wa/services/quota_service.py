...
class QuotaService:
    ...
    def get_unique_contacts_in_window(self, window_start, window_end, exclude_broadcast_id=None) -> set:
        ...
        return {
            ''.join(c for c in str(phone) if c.isdigit() or c == '+')
            for phone in phones
            if phone
        }
    ...
    def compute_quota(self, scheduled_time, exclude_broadcast_id=None) -> dict:
        ...
        contacts_after = self.get_unique_contacts_in_window(
            window_after_start, window_after_end, exclude_broadcast_id
        )
    ...
    def validate_broadcast(self, recipient_phones: list, scheduled_time=None, exclude_broadcast_id=None, open_session_phones: set = None) -> dict:
        """
    ...
    def get_contacts_with_active_csw(self, phone_list: list, at_time=None) -> set:
        ...
        tenant = self.wa_app.tenant
        contact_ids = TenantContact.objects.filter(
            tenant=tenant,
        ...
        contacts_with_csw = Messages.objects.filter(
            tenant=tenant,
        ...
        ).select_related('contact').values_list('contact__phone', flat=True).distinct()
