...
class TemplateNotificationService:
    ...
    def send_category_change_notification(template, old_category: str, new_category: str):
        ...
        try:
            ...
            recipient_emails = [tu.user.email for tu in tenant_users if tu.user.email and tu.user.is_active]
...
The Jina Connect Team
            ...
            logger.info(
                f"Category change notification sent to {len(recipient_emails)} users for template {template.element_name}"
            )
            return True
    ...
    def send_status_change_notification(template, old_status: str, new_status: str, reason: str = None):
        ...
        try:
            ...
            if new_status == "APPROVED":
                ...
                icon = "❌"
                status_message = (
                    "Your template has been rejected. Please review the reason below and make necessary changes."
                )
