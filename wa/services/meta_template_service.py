...
def get_validator_class(template_type: str) -> Type:
    ...
    from wa.utility.validators.meta_direct.create.mpm_template_request import \
    ...
    from wa.utility.validators.meta_direct.create.spm_template_request import \
...
class MetaTemplateService:
    ...
    def __init__(self, wa_app):
        ...
        if not self.waba_id:
            raise MetaTemplateServiceError(
                "WABA ID not configured. Please set up WABAInfo for this app.",
        ...
        if response.get('id'):
            ...
            raise MetaTemplateServiceError(
                ...
                errors=[{
                    ...
                    'type': f'meta_api_error_{error_code}'
                }],
                code=str(error_code)
            )
        else:
            raise MetaTemplateServiceError(
                f"Unexpected response from META API: {json.dumps(response)}",
    ...
    def _build_debug_info(
        ...
        debug_info = f"=== META DIRECT API TEMPLATE SUBMISSION ===\n"
        debug_info += f"Submitted at: {timezone.now().isoformat()}\n"
        debug_info += f"Provider: META Direct API\n"
    ...
    def _save_to_model(
        ...
        template_status = status_mapping.get(
            result.get('status', 'PENDING').upper(), 
    ...
    def _sync_with_bsp(self, template) -> None:
        ...
        try:
            sync_template_with_bsp_task.delay(
                template_id=template.pk,
                wa_id=self.wa_app.pk
            )
