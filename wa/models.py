...
from abstract.models import (BaseModel, BaseModelWithOwner,
                             BaseTenantModelForFilterUser, BaseWebhookDumps)
...
class WATemplate(BaseTemplateMessages):
    ...
    example_media_url = models.URLField(max_length=1024, blank=True, null=True, help_text="Example media URL for approval")
    ...
    def to_gupshup_payload(self) -> dict:
        ...
        * ``allowTemplateCategoryChange=false`` — prevents META from
        ...
        if body_text is not None:
            ...
            raise ValueError(
                "Body text is required for CAROUSEL templates. "
                "This text appears above the carousel cards."
            )
    ...
    def _convert_card_placeholders(self, card: dict) -> dict:
        ...
        if not card_copy.get('sampleText') and card_copy.get('body'):
            card_copy['sampleText'] = re.sub(
                r'\{\{\d+\}\}', '[sample]', card_copy['body']
            )
        ...
        # 'card_index', 'id', etc.).
        _GUPSHUP_CARD_KEYS = {
            'headerType', 'mediaUrl', 'mediaId', 'exampleMedia',
            'body', 'sampleText', 'buttons',
        }
        card_copy = {
            k: v for k, v in card_copy.items()
            if v is not None and k in _GUPSHUP_CARD_KEYS
        }
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.number:
            ...
            self.number = TemplateNumber.objects.create(
                name=self.element_name or self.name or ""
            )
...
class WAMessage(BaseTenantModelForFilterUser):
    ...
    contact = models.ForeignKey(TenantContact, on_delete=models.CASCADE, related_name='wa_messages', blank=True, null=True)
    ...
    # Media content
    media_id = models.CharField(max_length=255, blank=True, null=True, db_index=True, help_text="META media ID (from upload or webhook)")
    media_url = models.URLField(max_length=2048, blank=True, null=True, help_text="Media download URL (expires after 5 min)")
...
class WAWebhookEvent(BaseWebhookDumps):
    ...
    message = models.ForeignKey(WAMessage, on_delete=models.SET_NULL, blank=True, null=True, related_name='webhook_events')
...
class WAMedia(BaseTenantModelForFilterUser):
    ...
    category = models.CharField(
        max_length=20, choices=MediaCategory.choices,
    ...
    source = models.CharField(
        max_length=10, choices=MediaSource.choices, default=MediaSource.UPLOAD
    )
    ...
    used_in_template = models.ForeignKey(
        WATemplate, on_delete=models.SET_NULL, blank=True, null=True,
        related_name='media_assets',
        help_text="Template this media is used as header handle"
    )
    used_in_message = models.ForeignKey(
        WAMessage, on_delete=models.SET_NULL, blank=True, null=True,
        related_name='media_assets',
...
class TenantRateCard(BaseModel):
    ...
    def rate_change_percent(self):
        ...
        return ((self.reference_rate - self.previous_rate) / self.previous_rate * Decimal("100")).quantize(Decimal("0.01"))
...
class WAOrder(BaseTenantModelForFilterUser):
    ...
    # ── Relationships ──────────────────────────────────────────────────
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="wa_orders",
    )
    wa_app = models.ForeignKey(
        WAApp, on_delete=models.CASCADE, related_name="orders",
    )
    contact = models.ForeignKey(
        TenantContact, on_delete=models.CASCADE,
        related_name="wa_orders", blank=True, null=True,
    )
    outgoing_message = models.ForeignKey(
        "WAMessage", on_delete=models.SET_NULL,
        related_name="order_from_message", blank=True, null=True,
        help_text="The order_details WAMessage that created this order.",
    )
    order_status_messages = models.ManyToManyField(
        "WAMessage", related_name="order_status_updates", blank=True,
        help_text="All order_status WAMessages sent for this order.",
    )

    # ── Order Identification ───────────────────────────────────────────
    reference_id = models.CharField(
        max_length=35, db_index=True,
        help_text="Unique per tenant. Alphanumeric + _ . -",
    )
    order_type = models.CharField(
        max_length=20, choices=OrderType.choices, default=OrderType.PHYSICAL_GOODS,
    )
...
class WAPaymentEvent(BaseModel):
    ...
    order = models.ForeignKey(
        WAOrder, on_delete=models.CASCADE, related_name="payment_events",
    )
    webhook_event = models.ForeignKey(
        "WAWebhookEvent", on_delete=models.SET_NULL,
        related_name="payment_events", blank=True, null=True,
    )
