...
class TenantTransactionFilter(django_filters.FilterSet):
    ...
    def _build_historical_filter(self, queryset, **historical_filters):
        ...
        historical_subquery = HistoricalBroadcast.objects.filter(
            id=OuterRef('broadcast_id'),
            history_id=OuterRef('broadcast_history_id'),
            **historical_filters
        )
        ...
        return queryset.filter(
            Q(broadcast_history_id__isnull=False, broadcast__isnull=False) & 
            Q(Exists(historical_subquery)) |
