...
def handle_razorpay_order_changes(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        try:
            ...
            logger.info(
                f"Created transaction {transaction.system_transaction_id} for RazorPay order {instance.order_id}"
            )
        ...
        try:
            ...
            existing_transaction = TenantTransaction.objects.get(razor_pay_order=instance)
            ...
            if existing_transaction:
                ...
                logger.warning(
                    f"No existing transaction found for RazorPay order {instance.order_id}, creating new one"
                )
...
def update_transaction_based_on_razorpay_status(transaction, razorpay_order):
    ...
    if old_status in [TransactionTypeChoices.SUCCESS_RECHARGE]:
        logger.info(
            f"Transaction {transaction.system_transaction_id} already marked as successful recharge; no update needed"
        )
        return
