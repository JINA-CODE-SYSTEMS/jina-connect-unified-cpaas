"""Shared fixtures for crm tests (#201 review)."""
...
def tenant(db):
    ...
    return Tenant.objects.create(name=f"CrmTenant-{uuid.uuid4().hex[:8]}")
...
@pytest.fixture
def hubspot_connection(db, tenant):
    from crm.models import CrmConnection

    return CrmConnection.objects.create(
        tenant=tenant,
        name="hs",
        provider="hubspot",
        access_token="tkn",
...
@pytest.fixture
def salesforce_connection(db, tenant):
    from crm.models import CrmConnection
