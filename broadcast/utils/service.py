...
class BroadcastService(BaseModel):
    ...
    def create_batches(self, batch_size: int = 100) -> List[List[int]]:
        ...
        message_ids = list(
            BroadcastMessage.objects.filter(broadcast_id=self.broadcast_id)
            .values_list('id', flat=True)
        )
        
        #check if batch_size is more than total messages
        if batch_size >= len(message_ids):
            logger.info(f"Batch size {batch_size} is greater than or equal to total messages {len(message_ids)}. Creating single batch.")
            return [message_ids]
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            logger.info(f"Successfully created {len(created_messages)} message records for broadcast {self.broadcast_id}")
