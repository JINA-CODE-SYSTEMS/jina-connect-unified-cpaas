...
def compute_charge_breakdown_task(
    self, wa_app_id: int, contact_ids: list = None,
    broadcast_id: int = None, template_id=None,
):
    """
...
def _create_team_inbox_message_from_broadcast(broadcast_message) -> dict:
    ...
    try:
        ...
        logger.info(f"[_create_team_inbox_message_from_broadcast] Created Messages entry {message.pk} for broadcast message {broadcast_message.pk}")
                ...
                if result['success']:
                    ...
                    try:
                        ...
                        if inbox_result['created']:
                            logger.info(f"Created team inbox message {inbox_result['message_id']} for broadcast message {message.id}")
                        else:
                            logger.warning(f"Failed to create team inbox message for broadcast message {message.id}: {inbox_result.get('error')}")
                    except Exception as inbox_error:
                        logger.exception(f"Error creating team inbox message for broadcast message {message.id}: {str(inbox_error)}")
                else:
            ...
            try:
                with transaction.atomic():
                    BroadcastMessage.objects.filter(id__in=message_ids).update(
                        ...
                        response=f"Max retries exceeded: {str(exc)}"
                    )
...
def _get_wa_api_for_broadcast(message):
    ...
    if bsp == BSPChoices.META:
        from wa.utility.apis.meta.template_api import \
            TemplateAPI as MetaTemplateAPI
        ...
        if not app_id or not app_secret:
            raise ValueError(
                f"Gupshup credentials (app_id/app_secret) missing on WAApp {wa_app.pk}"
            )
    ...
    try:
        ...
        for comp in template_components:
            if comp.get('type') == 'CAROUSEL':
                logger.info(
                    f"CAROUSEL payload for {message.contact.phone}: "
                    f"{json.dumps(comp, indent=2, default=str)}"
                )
...
def handle_sms_message(message):
    ...
    try:
        ...
        return {
            'success': True,
            'message_id': simulated_message_id,
            'response': 'SMS sent successfully (simulated)'
        }
