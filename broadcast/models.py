...
class Broadcast(BaseTenantModelForFilterUser):
    ...
    RESERVED_VARS = [
        ...
        {
            'key': 'contact_name',
            'description': 'Contact name (full name or "Customer" if empty)'
        },
        {
            'key': 'phone',
        ...
        {
            'key': 'tenant_name',
            'description': 'Name of the tenant (alias for company_name)'
        },
        {
            'key': 'today_date',
            'description': 'Current date in format "November 05, 2025"'
        },
        {
            'key': 'current_time',
            'description': 'Current time in format "02:30 PM"'
        },
        {
            'key': 'current_year',
            'description': 'Current year as string'
        },
        {
            'key': 'broadcast_name',
    ...
    status = models.CharField(
        max_length=20,
        choices=BroadcastStatusChoices.choices,
        default=BroadcastStatusChoices.DRAFT
    )
    ...
    updated_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name='updated_broadcasts'
    )
    platform = models.CharField(
        max_length=20,
        choices=BroadcastPlatformChoices.choices,
        default=BroadcastPlatformChoices.WHATSAPP
    )
    task_id = models.CharField(max_length=255, blank=True, null=True, editable= False)
    template_number = models.ForeignKey(
        TemplateNumber, on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    credit_deducted = models.BooleanField(default=False, help_text="Whether credits have been deducted for this broadcast")
    refund_processed = models.BooleanField(default=False, help_text="Whether refund has been processed for failed messages")
    initial_cost = MoneyField(max_digits=10, decimal_places=2, default_currency='USD', null=True, blank=True, help_text="Initial cost deducted for the broadcast")
    refund_amount = MoneyField(max_digits=10, decimal_places=2, default_currency='USD', null=True, blank=True, help_text="Amount refunded for failed messages")
...
class BroadcastMessage(BaseTenantModelForFilterUser):
    ...
    def payload(self):
        ...
        if self.broadcast.platform == BroadcastPlatformChoices.WHATSAPP:
            ...
            raise NotImplementedError("Payload generation not implemented for platform {}".format(self.broadcast.platform))
    ...
    def _build_template_components(self):
        ...
        if not placeholder_mapping:
            ...
            if body_vars:
                placeholder_mapping = {
                    'content': {v: v for v in sorted(set(body_vars), key=int)}
                }

            # Even without placeholder_mapping, check for copy_code/OTP buttons
            if template.buttons:
                copy_code_components = self._build_copy_code_button_components(
                    template.buttons,
                    all_data
                )
    ...
    def _build_copy_code_button_components(self, template_buttons, all_data):
        ...
        for idx, button in enumerate(template_buttons):
            ...
            if is_otp_copy_code or is_marketing_copy_code:
                coupon_value = (
                    all_data.get('coupon_code') or
                    all_data.get('otp_code') or
                    all_data.get('code') or
                    all_data.get('otp') or
                    ''
                )
                ...
                try:
                    ...
                    if override_tm.media:
                        ...
                        card_component["components"].append({
                            "type": "header",
                            "parameters": [{
                                "type": override_media_type,
                                override_media_type: {"link": media_url}
                            }]
                        })
            ...
            if not card_override_id:
                ...
                if not media_url:
                    ...
                    if card_example_url:
                        ...
                        logger.info(
                            f"Card {card_index}: no TenantMedia, using "
                            f"example_media_url fallback"
                        )
    ...
    def _build_media_header_component(self, template):
        ...
        MEDIA_TYPE_CONFIG = {
            ...
            TemplateType.DOCUMENT: {
                'param_type': 'document',
                'extra_fields': lambda media: {
                    'filename': media.name.split('/')[-1] if media.name else 'document.pdf'
                }
            },
            TemplateType.VIDEO: {
                'param_type': 'video',
        ...
        if override_media_id:
            try:
                ...
                if override_tm.media:
                    ...
                    media_param = {
                        'link': media_url,
                        **config['extra_fields'](override_tm.media)
                    }
        ...
        if not media_param and template.tenant_media and template.tenant_media.media:
            ...
            media_param = {
                'link': media_url,
                **config['extra_fields'](template.tenant_media.media)
            }
    ...
    def _wa_payload(self):
        ...
        if self.broadcast.template_number:
            from wa.models import WATemplate, TemplateType
            template: WATemplate = self.broadcast.template_number.gupshup_template
            ...
            has_content_placeholders = (
                ('{{' in (template.content or '')) or
                ('{{' in (template.header or ''))
            )
            if (template.placeholder_mapping or has_copy_code_buttons
                    or is_carousel or has_media_header
