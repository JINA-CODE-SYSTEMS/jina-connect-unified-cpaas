...
from broadcast.services.credit_manager import (BroadcastCreditManager,
...
def cache_broadcast_old_state(sender, instance, **kwargs):
    ...
    if instance.pk:  # Only for updates, not new creations
        try:
            ...
            logger.debug(f"[PRE_SAVE] Cached old state for broadcast {instance.pk}: status={old_instance.status}, task_id={old_instance.task_id}")
...
def handle_broadcast_scheduling(sender, instance, created, update_fields, **kwargs):
    ...
    if created:
        ...
        if instance.status not in instance.green_signal_stages:
            logger.debug(f"[CREATION] Broadcast {instance.id} created with status {instance.status}, no scheduling needed")
            return
        ...
        if instance.scheduled_time < instance.threshold_time_from_now:
            logger.warning(f"[VALIDATION ERROR] Broadcast {instance.id}: Schedule time must be at least {settings.BROADCAST_CANCELLATION_TIME_LIMIT_IN_MINUTES} minutes ahead")
            Broadcast.objects.filter(pk=instance.id).update(
                status=BroadcastStatusChoices.FAILED,
        ...
        if status_changed and (instance.status == BroadcastStatusChoices.CANCELLED or (old_was_green and not new_is_green)):
        ...
        # Handle status change from non-green to green (e.g., CANCELLED -> SCHEDULED, DRAFT -> QUEUED)
            ...
            if instance.good_to_send:
                logger.debug(f"[UPDATE] Broadcast {instance.id} changed from {old_state['status']} to {instance.status}, scheduling...")
    ...
    try:
        ...
        try:
            ...
            Broadcast.objects.filter(pk=broadcast_instance.pk).update(
                status=BroadcastStatusChoices.FAILED,
                reason_for_cancellation=error_msg
            )
            return
        ...
        task_result = setup_broadcast_task.apply_async(
            args=[broadcast_instance.id],
            countdown=countdown
        )
        ...
        logger.debug(
            f"[TASK SCHEDULED] Broadcast {broadcast_instance.id}: "
            f"Task {task_result.id} in {countdown}s"
        )
...
def validate_broadcast_schedule(broadcast_instance):
    ...
    if broadcast_instance.scheduled_time < broadcast_instance.threshold_time_from_now:
        return False, f"Scheduled time must be at least {settings.BROADCAST_CANCELLATION_TIME_LIMIT_IN_MINUTES} minutes in the future"
