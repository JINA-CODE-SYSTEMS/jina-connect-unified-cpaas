...
class BroadcastSerializer(BaseSerializer):
    ...
    tag_filter = serializers.ListField(
        child=serializers.CharField(), write_only=True, default=list, required=False
    )
    ...
    def create(self, validated_data):
        ...
        logger.debug(f"[SERIALIZER CREATE] template_number in validated_data: {validated_data.get('template_number')!r}")
    ...
    def save(self, **kwargs):
        ...
        if getattr(instance, '_select_all', False):
            ...
            qs = TenantContact.objects.filter(
                tenant=instance.tenant, phone__isnull=False
            ).exclude(phone='')
        ...
        if has_deferred:
            ...
            logger.debug(f"[SERIALIZER SAVE] instance.save() completed. DB status now: {Broadcast.objects.values_list('status', flat=True).get(pk=instance.pk)}")
    ...
    def update(self, instance, validated_data):
        ...
        if instance.status not in instance.allowed_status_for_update:
            raise ValidationError(
                {"status": f"Cannot update broadcast in '{instance.get_status_display()}' status."}
            )
...
class BroadcastLimitedSerializer(BroadcastSerializer):
    ...
    class Meta(BroadcastSerializer.Meta):
        exclude = ['initial_cost', 'initial_cost_currency',
