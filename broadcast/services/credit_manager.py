...
class BroadcastCreditManager:
    ...
    def process_refund_for_broadcast(broadcast: Broadcast):
        ...
        if broadcast.status == BroadcastStatusChoices.CANCELLED:
            ...
            logger.info("[REFUND] Broadcast %s completed. Processing refund for %s failed messages.", broadcast.id, failed_count)
        else:
        ...
        if refund_amount > broadcast.initial_cost.amount:
            logger.warning(f"[REFUND WARNING] Calculated refund ({refund_amount}) > initial cost ({broadcast.initial_cost})")
            refund_amount = broadcast.initial_cost.amount
        ...
        if refund_amount <= 0.0:
            ...
            Broadcast.objects.filter(pk=broadcast.pk).update(
                ...
                refund_amount=Money(0, broadcast.tenant.balance.currency)
            )
            return None
        ...
        with db_transaction.atomic():
            ...
            Broadcast.objects.filter(pk=broadcast.pk).update(
                refund_processed=True,
                refund_amount=Money(refund_amount, broadcast.tenant.balance.currency)
            )
