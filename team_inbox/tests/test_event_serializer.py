...
from team_inbox.models import Event, EventTypeChoices, MessageEventIds
...
class EventSerializerTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user_a = User.objects.create_user(
            username="evt_a", email="evt_a@t.com",
            mobile="+910000099991", password="testpass123",
            first_name="Alice", last_name="A",
        )
        cls.user_b = User.objects.create_user(
            username="evt_b", email="evt_b@t.com",
            mobile="+910000099992", password="testpass123",
            first_name="Bob", last_name="B",
        )
        TenantUser.objects.create(
            user=cls.user_a, tenant=cls.tenant,
            role=cls.owner_role, is_active=True,
        )
        cls.contact = TenantContact.objects.create(
            tenant=cls.tenant, phone="+910000099993",
    ...
    def test_null_actors(self):
        ...
        evt = self._create_event(
            assigned_by_type=None, assigned_by_id=None, assigned_by_user=None,
            assigned_to_type=None, assigned_to_id=None, assigned_to_user=None,
        )
        data = EventSerializer(evt).data
