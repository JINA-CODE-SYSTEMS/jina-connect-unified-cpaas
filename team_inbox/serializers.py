...
from team_inbox.models import (AuthorChoices, Event, MessageDirectionChoices,
...
class MessagesSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Messages
        fields = [
            'id', 'message_id', 'content', 'timestamp', 'direction', 
            'platform', 'author', 'tenant', 'created_at', 'updated_at',
            'expires_at', 'is_read', 'read_at', 'read_by_id', 'read_by_name',
            'outgoing_status', 'outgoing_sent_at', 'outgoing_delivered_at',
            'outgoing_read_at', 'outgoing_failed_at'
        ]
        read_only_fields = [
            'id', 'timestamp', 'created_at', 'updated_at', 'expires_at', 
            'is_read', 'read_at', 'read_by_id', 'read_by_name',
            'outgoing_status', 'outgoing_sent_at', 'outgoing_delivered_at',
            'outgoing_read_at', 'outgoing_failed_at'
        ]
...
class EventSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Event
        fields = [
            'id', 'event_type', 'event_type_display', 'note',
            'created_by_name', 'assigned_by_name', 'assigned_to_name',
            'icon', 'color_background', 'color_text', 'event_data',
...
class MessageCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Messages
        fields = [
            'message_id', 'content', 'direction', 'platform', 'author'
        ]
...
class MessageListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Messages
        fields = [
            'id', 'message_id', 'content_preview', 'timestamp', 
            'direction', 'platform', 'author'
        ]
...
class WebSocketMessageSerializer(serializers.Serializer):
    ...
    platform = serializers.ChoiceField(
        choices=MessagePlatformChoices.choices, 
        required=False, 
        default=MessagePlatformChoices.WHATSAPP
    )
