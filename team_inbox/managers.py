...
from django.db.models import (Case, CharField, F, IntegerField, Max, OuterRef,
                              Q, Subquery, Value, When)
from django.db.models.functions import Cast, Coalesce, Concat, Extract, Left
...
class MessageEventIdsQuerySet(models.QuerySet):
    ...
    def for_contact(self, contact):
        ...
        """
        return self.filter(
            Q(messages__contact=contact) | Q(events__contact=contact)
        ).distinct()
    ...
    def for_tenant(self, tenant):
        ...
        """
        return self.filter(
            Q(messages__tenant=tenant) | Q(events__tenant=tenant)
        ).distinct()
    ...
    def get_timeline_for_contact(self, contact, limit=50, offset=0, include_date_separators=True):
        ...
        if message_ids:
            messages = Messages.objects.filter(
                id__in=message_ids
            ).select_related('message_id', 'tenant_user', 'contact')
        ...
        return self.annotate(
            expires_at_timestamp=Case(
                When(
                    platform='WHATSAPP',
                    then=Cast(
                        Extract('timestamp', 'epoch') + Value(twenty_four_hours),
...
class MessagesManager(BaseTenantModelForFilterUserManager):
    ...
    def get_chat_list(self, tenant_id, limit=50, offset=0, search=None):
        ...
        chat_list = []
        for row in rows:
            (message_id, contact_id, content, timestamp, direction, 
             author, platform, tenant_user_id, is_read, contact_first_name, 
             contact_last_name, phone, contact_status, assigned_to_type,
             assigned_to_id, assigned_to_user_id, user_first_name, user_last_name,
             assigned_user_first_name, assigned_user_last_name, assigned_user_email,
             unread_count, last_incoming_timestamp) = row
    ...
    def _get_assigned_to_name(self, assigned_to_type, assigned_to_id, 
    ...
    def _get_created_by(self, author, user_first_name, user_last_name, 
