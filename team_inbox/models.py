...
class Messages(BaseTenantModelForFilterUser):
    ...
    message_id = models.ForeignKey(MessageEventIds, on_delete=models.CASCADE, related_name='messages', null=True, blank=True)
...
class Event(BaseTenantModelForFilterUser):
    ...
    # Core fields
    event_id = models.ForeignKey(MessageEventIds, on_delete=models.CASCADE, related_name='events', null=True, blank=True)
    tenant = models.ForeignKey(
        Tenant,
        on_delete=models.CASCADE,
        related_name='events'
    ...
    created_by_type = models.CharField(
        max_length=10,
        choices=ActorTypeChoices.choices,
        default=ActorTypeChoices.USER
    )
    ...
    event_data = models.JSONField(
        default=dict,
        blank=True,
        help_text="Type-specific event data (reason, resolution_notes, etc.)"
    )
