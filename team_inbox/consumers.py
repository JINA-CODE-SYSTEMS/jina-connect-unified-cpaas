...
from tenants.models import DefaultRoleSlugs, Tenant, TenantUser
...
class TeamInboxConsumer(AsyncWebsocketConsumer):
    ...
    def get_contact(self, contact_id: int) -> Optional[TenantContact]:
        ...
        """
        try:
            return TenantContact.objects.get(
                id=contact_id,
                tenant_id=self.tenant_id
            )
    ...
    @database_sync_to_async
    def get_timeline_for_contact(self, contact: TenantContact, limit: int = 50, offset: int = 0, include_date_separators: bool = True):
        """
        ...
        try:
            ...
            timeline = MessageEventIds.objects.get_timeline_for_contact(
                ...
                limit=limit,
                offset=offset,
                include_date_separators=include_date_separators
            )
    ...
    def get_chat_list(self, limit: int = 50, offset: int = 0, search: str = None):
        ...
        """
        try:
            return Messages.objects.get_chat_list(
                tenant_id=self.tenant_id,
                limit=limit,
                offset=offset,
                search=search
    ...
    def mark_contact_messages_as_read(self, contact_id: int) -> list:
        ...
        try:
            ...
            unread_ids = list(Messages.objects.filter(
                contact_id=contact_id,
                tenant_id=self.tenant_id,
                direction='INCOMING',
                is_read=False
...
class NotificationConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        ...
        logger.info(
            f"User {getattr(self.user, 'id', 'Unknown')} disconnected from notifications (code: {close_code})"
        )
