...
from team_inbox.serializers import (MessageCreateSerializer,
                                    MessageListSerializer, MessagesSerializer)
...
class MessagesViewSet(BaseTenantModelViewSet):
    ...
    def platforms(self, request):
        ...
        stats = self.get_queryset().values('platform').annotate(
            count=Count('id')
        ).order_by('-count')
    ...
    def bulk_delete(self, request):
        ...
        return Response({
            'status': f'{deleted_count} messages deleted',
